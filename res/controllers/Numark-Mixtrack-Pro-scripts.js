...
NumarkMixTrackPro.flashLED = function(led, veces) {
    ...
    const func = "NumarkMixTrackPro.doFlash(" + ndx + ", " + veces + ")";
    const id = engine.beginTimer(120, func);
...
NumarkMixTrackPro.flanger = function(channel, control, value, status, group) {
    ...
    const deck = NumarkMixTrackPro.groupToDeck(group);
...
NumarkMixTrackPro.repositionHack = function(group, oldPosition) {
    ...
    const bar = NumarkMixTrackPro.samplesPerBeat(group);
