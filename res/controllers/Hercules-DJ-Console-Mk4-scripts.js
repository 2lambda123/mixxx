...
HerculesMk4.init = function(id) {
    // Switch off all LEDs
    for (let i=1; i<95; i++) { midi.sendShortMsg(0x90, i, 0x00); }
...
HerculesMk4.pitch = function(midino, control, value, status, group) {
    ...
    if (HerculesMk4.scratchButton == 1) {
        ...
        const newValue = HerculesMk4.knobIncrement(group, "pregain", 0, 4, 1, 60, sign);
        engine.setValue(group, "pregain", newValue);
    } else {

        let increment = 0.005 * HerculesMk4.sensitivityPitch[HerculesMk4.deck(group)];
...
HerculesMk4.pfl = function(midino, control, value, status, group) {
    ...
    if (value) {
        ...
        const pfl1=engine.getValue("[Channel1]", "pfl");
        const pfl2=engine.getValue("[Channel2]", "pfl");


        const actualMixCue=engine.getValue("[Master]", "headMix");
...
HerculesMk4.pitchbend = function(midino, control, value, status, group) {
    ...
    if (control == 0x0B || control == 0x1F) { // Pitchbend +
        const newValue = (HerculesMk4.jogFastPosition[HerculesMk4.deck(group)]==1)? 0 : 1;
...
HerculesMk4.jogWheel = function(midino, control, value, status, group) {
    ...
    const turboSeek=engine.getValue(group, "play")==0 && HerculesMk4.jogFastPosition[HerculesMk4.deck(group)]==1 && HerculesMk4.scratchMode[HerculesMk4.deck(group)]==0;

    if (turboSeek) {
        let new_position = engine.getValue(group, "playposition")+ 0.008*direction;
