...
Jockey3ME.wheelTouch = function(channel, control, value, status, group) {
    const currentDeck = parseInt(group.substring(8, 9));
...
Jockey3ME.effectParam = function(channel, control, value, status, group) {
    const currentDeck = parseInt(group.substring(23, 24));
    let newVal = 0;
    const interval = 0.04;
    let EncoderKnobFX = control - 29;
    const curVal = engine.getParameter("[EffectRack1_EffectUnit" + currentDeck + "_Effect1]", "parameter" + EncoderKnobFX);
    if (value == 0x41) {
...
Jockey3ME.effectDryWet = function(channel, control, value, status, group) {
    ...
    let newValue = 0;
    const interval = 0.04;
    const curVal = engine.getParameter("[EffectRack1_EffectUnit" + currentDeck + "]", "mix");
    if (value == 0x41) {
...
Jockey3ME.FX_Param_Led = function(group, value, control) {
    const currentDeck = parseInt(value.substring(23, 24)); // Why i have to use value and not group?
    const knob = parseInt(control.substring(9, 10));
    const newValue = engine.getParameter(value, control);
    midi.sendShortMsg(0x90+(currentDeck-1), 0x1E+(knob-1), (newValue*127));
};

Jockey3ME.FX_DryWet_Led = function(group, value, control) {
    const currentDeck = parseInt(value.substring(23, 24));
    const newValue = engine.getParameter(value, control);
...
Jockey3ME.effectSelectLedSetNumEffect = function(currentDeck, status, control, value) {
    ...
    const ledValue = Jockey3ME.effectsAvailable; // num_effects returns how many effects in Unit is, not what is available. Set to 5
...
Jockey3ME.effectSelect = function(channel, control, value, status, group) {
    ...
    if (fxChainSelectKnob) {
        ...
        let num_parameters = engine.getValue("[EffectRack1_EffectUnit" + currentDeck + "_Effect1]", "num_parameters");
        if (num_parameters > 3) {
            num_parameters = 3;
        }
        for (let i = 1; i <= num_parameters; i++) {
    ...
    if (value == 0x00) {
        ...
        if (engine.getValue("[EffectRack1_EffectUnit" + currentDeck + "_Effect" + Jockey3ME.fxSelectKnobPush[currentDeck-1] + "]", "loaded")) {
            fxSelectKnobPushBreak: {
                for (let r = 0, s = 1; r < Jockey3ME.fxSelectKnobPushIterator.length; r++) {
                    if ((r % 4) == 0 && r != 0) {
                        ++s;
                    }
                    let variableSvalue = 0;
                    if (s == 2) {
                        variableSvalue = 4;
                    } else if (s == 3) {
                        variableSvalue = 8;
                ...
                let currentDeckKnob = currentDeck;
...
Jockey3ME.effectSuperKnob = function(channel, control, value, status, group) {
    ...
    const currentDeck = parseInt(group.substring(23, 24));
    const interval = 0.00065;
    const curVal = engine.getParameter("[EffectRack1_EffectUnit" + currentDeck + "]", "super1");
...
Jockey3ME.ShiftTraxEncoderTurn = function(channel, control, value, status, group) {
    ...
    if (newValue == 1) { engine.setValue(group, "SelectNextPlaylist", newValue); } else { engine.setValue(group, "SelectPrevPlaylist", 1); }
};
...
Jockey3ME.MixerVol = function(channel, control, value, status, group) {
    let currentDeck = parseInt(group.substring(8, 9));
...
Jockey3ME.EQ = function(channel, control, value, status, group) {
    let currentDeck = parseInt(group.substring(24, 25));
    if ((Jockey3ME.MixerDeck1 == 1 && currentDeck == 1) || (Jockey3ME.MixerDeck2 == 1 && currentDeck == 2)) {
        currentDeck += 2;
    }
    switch (control) {
