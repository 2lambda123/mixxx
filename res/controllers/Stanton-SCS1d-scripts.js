...
StantonSCS1d.scratch = {
    "revtime": (60/StantonSCS1d.rpm[StantonSCS1d.platterSpeed]),
    "resolution": 4000, "alpha": 1.0/8, "beta": (1.0/8)/32,
    "prevTimeStamp": 0, "prevState": 0
};
// Pitch values for key change mode
StantonSCS1d.pitchPoints = {
    1: {
        8: -0.1998, 9: -0.1665, 10: -0.1332, 11: -0.0999, 12: -0.0666, 13: -0.0333,
        14: 0.0333, 15: 0.0666, 18: 0.0999, 19: 0.1332, 20: 0.1665, 21: 0.1998
    }, // 3.33% increments
    2: {
        8: -0.5, 9: -0.4043, 10: -0.2905, 11: -0.1567, 12: -0.1058, 13: -0.0548,
        14: 0.06, 15: 0.12, 18: 0.181, 19: 0.416, 20: 0.688, 21: 1.0
    },  // Key changes
    3: {
        8: -0.4370, 9: -0.3677, 10: -0.3320, 11: -0.2495, 12: -0.1567, 13: -0.0548,
        14: 0.12, 15: 0.263, 18: 0.338, 19: 0.506, 20: 0.688, 21: 0.895
    }
};  // Notes
// Multiple banks of multiple cue points:
StantonSCS1d.hotCues = {
    1: {0x20: 1, 0x21: 2, 0x22: 3, 0x23: 4},
    2: {0x20: 5, 0x21: 6, 0x22: 7, 0x23: 8},
    3: {0x20: 9, 0x21: 10, 0x22: 11, 0x23: 12}
};
// Button-to-index mapping for trigger section
StantonSCS1d.trigButtons = {
    8: 0,  9: 1,  10: 2,  11: 3,
    12: 4, 13: 5, 14: 6,  15: 7,
    18: 8, 19: 9, 20: 10, 21: 11
};
// Button-to-reloop index mapping for trigger section
StantonSCS1d.trigButtonsLoopNr = {
    8: 1,  9: 1,  10: 2, 11: 2,
    12: 3, 13: 3, 14: 4, 15: 4,
    18: 5, 19: 5, 20: 6, 21: 6
};
...
StantonSCS1d.platterSignals = {
    "vinyl": [["CurrentChannel", "rate", "StantonSCS1d.platterSpeed"]],
    "control": [],
    "browse": [],
...
StantonSCS1d.init = function(id, debug) {
    ...
    const CC = 0xB0 + StantonSCS1d.channel;
    const No = 0x90 + StantonSCS1d.channel;
...
StantonSCS1d.init2 = function() {
    ...
    if (!StantonSCS1d.platterMode["[Channel"+StantonSCS1d.deck+"]"]) { StantonSCS1d.platterMode["[Channel"+StantonSCS1d.deck+"]"] = StantonSCS1d.platterMode.default; }
...
StantonSCS1d.inboundSysex = function(data, length) {
    if (data[3]==StantonSCS1d.sysex[3]) {
        ...
        if (data[4]==0x48 && data[5]==0x53 && data[6]==0x53) {
            ...
            // Platter message
            const offset=2;   // TODO: once Clemens removes the bogus 00, remove this
            const message = [];
...
StantonSCS1d.connectKnobSignals = function(channel, disconnect) {
    ...
    const signalList = StantonSCS1d.knobSignals[StantonSCS1d.knobMode["[Channel"+StantonSCS1d.deck+"]"]-1];
    ...
    for (let i=0; i<signalList.length; i++) {
        ...
        if (!disconnect) {
            // Alternate:
            const command = signalList[i][2]+"("+engine.getValue(group, signalList[i][1])+")";
            //print("StantonSCS1d: command="+command);
            eval(command);

            // Set text
            const message = StantonSCS1d.knobText[StantonSCS1d.knobMode["[Channel"+StantonSCS1d.deck+"]"]-1][i];
        ...
        if (StantonSCS1d.debug) {
            if (disconnect) { print("StantonSCS1d: "+group+","+signalList[i][1]+" disconnected from "+signalList[i][2]); } else { print("StantonSCS1d: "+group+","+signalList[i][1]+" connected to "+signalList[i][2]); }
        }
    }
...
StantonSCS1d.connectPadSignals = function(channel, disconnect) {
    ...
    const deck = StantonSCS1d.padBank.deck;
    let bank = StantonSCS1d.padBank["bank"+deck];
    // Set the default bank if not present
    if (!bank) { bank = StantonSCS1d.padBank["bank"+deck] = StantonSCS1d.padBank.default; }

    const signalList = StantonSCS1d.padSignals[bank];
    for (let i=0; i<signalList.length; i++) {
        let group = signalList[i][0];
...
StantonSCS1d.connectTrigSignals = function(channel, disconnect) {
    ...
    let bank = StantonSCS1d.triggerBank[StantonSCS1d.deck];
    // Set the default bank if not present
    if (!bank) { bank = StantonSCS1d.triggerBank[StantonSCS1d.deck] = StantonSCS1d.triggerBank.default; }

    const signalList = StantonSCS1d.trigSignals[bank];
    ...
    if (disconnect) {
        const byte1 = 0x80 + channel;
        for (var i=8; i<=15; i++) { midi.sendShortMsg(byte1, i, 0x00); }
        for (var i=18; i<=21; i++) { midi.sendShortMsg(byte1, i, 0x00); }
    }
};
...
StantonSCS1d.connectDeckSignals = function(channel, disconnect) {
    ...
    const signalList = StantonSCS1d.deckSignals;
    for (let i=0; i<signalList.length; i++) {
        let group = signalList[i][0];
...
StantonSCS1d.playButton = function(channel, control, value, status) {
    if ((status & 0xF0) != 0x80) {    // If button down
        ...
        if (StantonSCS1d.modifier.pad==1 || StantonSCS1d.modifier.cue==1) { midi.sendShortMsg(0x90+channel, control, 127); }    // Make it orange

        const currentlyPlaying = engine.getValue("[Channel"+StantonSCS1d.deck+"]", "play");
...
StantonSCS1d.rangeButton = function(channel, control, value, status) {
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (StantonSCS1d.crossFader) {
            ...
            const xfader = engine.getValue("[Master]", "crossfader")*63+64;
        ...
        if (!StantonSCS1d.state.crossfaderAdjusted) {
            // Change the range
            const currentRange = engine.getValue("[Channel"+StantonSCS1d.deck+"]", "rateRange");
            const high = StantonSCS1d.pitchRanges.length;
            if (currentRange<StantonSCS1d.pitchRanges[0] || currentRange>=StantonSCS1d.pitchRanges[high-1]) { engine.setValue("[Channel"+StantonSCS1d.deck+"]", "rateRange", StantonSCS1d.pitchRanges[0]); } else {
                let i=1;
                while (i<high) {
...
StantonSCS1d.vinylMoved = function(data, length) {
    ...
    const iInfo = (data[0] << 24) | (data[1] << 16) |
                (data[2] << 8) | data[3];
    ...
    const iTimeStamp = (iInfo >>> 8);
    const iState = data[3];
    if (isNaN(iState)) { return; }
    const iMoved = (iState - StantonSCS1d.scratch.prevState + 384) % 256 - 128;
    const iSpeed = iMoved/(iTimeStamp - StantonSCS1d.scratch.prevTimeStamp);
    ...
    const platterMode = StantonSCS1d.platterMode["[Channel"+StantonSCS1d.deck+"]"];
    ...
    case "browse":
        if (StantonSCS1d.state.browseTicks==StantonSCS1d.browseDamp) {
            ...
            if (iMoved>0) { engine.setValue("[Playlist]", "SelectNextTrack", 1); } else if (iMoved<0) { engine.setValue("[Playlist]", "SelectPrevTrack", 1); }
...
StantonSCS1d.DeckChange = function(channel, control, value, status) {
    ...
    if (new Date() - StantonSCS1d.modifier.deckTime>StantonSCS1d.deckChangeWait) {
        ...
        if (StantonSCS1d.globalMode) { StantonSCS1d.newPlatterMode = StantonSCS1d.platterMode["[Channel"+StantonSCS1d.deck+"]"]; }

        // Supports n-decks
        if (StantonSCS1d.deck == engine.getValue("[Master]", "num_decks")) { StantonSCS1d.deck=1; } else { StantonSCS1d.deck++; }
        ...
        let deckNr = StantonSCS1d.deck+"";
        deckNr = deckNr.toInt();
        if (StantonSCS1d.deck>9) { deckNr = 64+(StantonSCS1d.deck-9); }
        midi.sendSysexMsg(StantonSCS1d.sysex.concat([channel, 36, deckNr, 0xF7]), 8);

        if (StantonSCS1d.globalMode) { StantonSCS1d.knobMode["[Channel"+StantonSCS1d.deck+"]"] = StantonSCS1d.state.Oldknob; } else {
...
StantonSCS1d.encoderBank = function(channel, control, value, status) {
    ...
    let newMode = StantonSCS1d.knobMode["[Channel"+StantonSCS1d.deck+"]"];
...
StantonSCS1d.encoder1 = function(channel, control, value, status) {
    if (StantonSCS1d.checkInSetup()) { return; }
    let offset = (value-64);
    const knobMode = StantonSCS1d.knobMode["[Channel"+StantonSCS1d.deck+"]"];
    switch (knobMode) {
    case 1: // Low EQ
        switch (status & 0xF0) {
...
StantonSCS1d.encoderSet = function(mode, knob, offset, min, max) {
    const signalList = StantonSCS1d.knobSignals[mode-1];
    let group = signalList[knob-1][0];
    if (group=="CurrentChannel") { group = "[Channel"+StantonSCS1d.deck+"]"; }
    let newValue=engine.getValue(group, signalList[knob-1][1])+offset;
...
StantonSCS1d.encoderSetAbs = function(mode, knob, value, save) {
    ...
    if (value=="previous") { engine.setValue(group, key, StantonSCS1d.prevValues[key]); } else { engine.setValue(group, key, value); }
};

// ----------   Sliders  ----------

StantonSCS1d.pitchSlider = function(channel, control, value) {
    const currentValue = engine.getValue("[Channel"+StantonSCS1d.deck+"]", "rate");
    let newValue = (value-64)/63;
...
StantonSCS1d.triggerBankSelect = function(channel, control, value, status) {
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (lastBank != StantonSCS1d.triggerBank[StantonSCS1d.deck]
            ...
            for (var i=8; i<=15; i++) { midi.sendShortMsg(byte1, i, 0x01); } // Turn on button light
            for (var i=18; i<=21; i++) { midi.sendShortMsg(byte1, i, 0x01); } // Turn on button light
        }
...
StantonSCS1d.triggerButton = function(channel, control, value, status) {
    ...
    const byte1 = 0x90 + channel;
    const deck = StantonSCS1d.deck;

    const index = StantonSCS1d.triggerBank[deck];
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (index == 1) { // bank 1 = Looping
            ...
            const prevLoopNumber = StantonSCS1d.modifier["loop"+loopNumber];
            ...
            if (!StantonSCS1d.looseLoops && StantonSCS1d.modifier.trigger==index) {
                if (control % 2 == 0) { engine.setValue("[Channel"+deck+"]", "loop_start_position", -1); } else { engine.setValue("[Channel"+deck+"]", "loop_end_position", -1); }
                ...
                if (control % 2 == 0) {
                    if (!StantonSCS1d.looseLoops
                        && engine.getValue("[Channel"+deck+"]", "loop_start_position") != -1) { return; }
                    engine.setValue("[Channel"+deck+"]", "loop_in", 1);
                }
                if (control % 2 != 0) {
                    if (!StantonSCS1d.looseLoops
                        && engine.getValue("[Channel"+deck+"]", "loop_end_position") != -1) { return; }
            ...
            //    break;
            //}
            if (StantonSCS1d.pitchPoints[index][control] == -0.1) { StantonSCS1d.pitchPoints[index][control] = engine.getValue("[Channel"+deck+"]", "rate"); } else {
    ...
    if (index == 1) { // bank 1 = Looping
        ...
        if (StantonSCS1d.looseLoops || (!StantonSCS1d.looseLoops && StantonSCS1d.modifier.trigger!=index)) {
            if (control % 2 == 0) { engine.setValue("[Channel"+deck+"]", "loop_in", 0); } else { engine.setValue("[Channel"+deck+"]", "loop_out", 0); }
        }
    } else {
...
StantonSCS1d.pad = function(channel, control, value, status) {
    const deck = StantonSCS1d.padBank.deck;
    const bank = StantonSCS1d.padBank["bank"+deck];
    const hotCue = StantonSCS1d.hotCues[bank][control];
    const byte1 = 0x90 + channel;
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (StantonSCS1d.padVelocity && engine.getValue("[Channel"+deck+"]", "hotcue_"+hotCue+"_enabled")) { engine.setValue("[Channel"+deck+"]", "volume", (value/0x7A)); }   // 0x7A seems the highest # the unit sends
...
StantonSCS1d.padBankButton = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {  // If button down
        ...
        if (StantonSCS1d.padBank.deck!=deck) {
            ...
            if (StantonSCS1d.padBank["bank"+deck]==3) { StantonSCS1d.padBank["bank"+deck]=1; } else { StantonSCS1d.padBank["bank"+deck]++; }
    ...
    if (StantonSCS1d.padBank.deck!=deck) { midi.sendShortMsg(0x80 + channel, control, 0); }    // off
        ...
        switch (StantonSCS1d.padBank["bank"+deck]) {
        case 1:
...
StantonSCS1d.pitchChange = function(value, noScratchEnable) {
    const CC = 0xB0 + StantonSCS1d.channel;
    const group = "[Channel"+StantonSCS1d.deck+"]";
    ...
    const multiplier = value * engine.getValue(group, "rateRange");
    let iMotorPitch = 1000+(1000*multiplier);
...
StantonSCS1d.buttonLED = function(value, note, on, off) {
    if (value>0) { midi.sendShortMsg(0x90 + StantonSCS1d.channel, note, on); } else { midi.sendShortMsg(0x80 + StantonSCS1d.channel, note, off); }
};
...
StantonSCS1d.activateLoop = function(value) {
    ...
    if (value>0) {
        if (engine.getValue("[Channel"+StantonSCS1d.deck+"]", "loop_enabled")) { StantonSCS1d.loopActive[StantonSCS1d.deck]=1; } else { StantonSCS1d.loopActive[StantonSCS1d.deck]=-1; }
    }
};
...
StantonSCS1d.loopFlash = function(end) {
    const buttons = [0, 8, 10, 12, 14, 18, 20];
    let target;

    const activeLoop = StantonSCS1d.loopActive[StantonSCS1d.deck];
...
StantonSCS1d.trigLED = function(value, control) {
    if (value == -1) { StantonSCS1d.buttonLED(0, control, 0x7f, 0); } else { StantonSCS1d.buttonLED(1, control, 0x7f, 0); }
};
...
StantonSCS1d.PadDisplay = function(value, pad) {
    ...
    const deck = StantonSCS1d.padBank.deck;
    const bank = StantonSCS1d.padBank["bank"+deck];
    const hotCue = StantonSCS1d.hotCues[bank][pad];

    let message= "Hotcue"+(hotCue < 10 ? " " + hotCue : hotCue);    // If empty, just display hot cue #

    if (value!=-1) {
        // Set display with cue point time
        const samplerate = engine.getValue("[Channel"+deck+"]", "track_samplerate");
        const msecs = ((value/2/samplerate)*1000) | 0;    // OR with 0 replaces Math.floor and is faster
...
StantonSCS1d.pitchRangeLEDs = function(value) {
    ...
    const on = 0x90 + StantonSCS1d.channel;
    const off = 0x80 + StantonSCS1d.channel;
    switch (true) {
    case (value<=0.07):
...
StantonSCS1d.circleBars = function(value) {
    ...
    if (StantonSCS1d.trackDuration[deck]>30) {
        const trackTimeRemaining = ((1-value) * StantonSCS1d.trackDuration[deck]) | 0;
    ...
    const revtime = StantonSCS1d.scratch.revtime;
    const currentTrackPos = value * StantonSCS1d.trackDuration[deck];

    const revolutions = currentTrackPos/revtime;
    //     var light = ((revolutions-(revolutions|0))*18)|0;    // Use this for two bars
    const light = ((revolutions-(revolutions|0))*36)|0;   // OR with 0 replaces Math.floor and is faster
    ...
    const byte1 = 0xB0 + StantonSCS1d.channel;
