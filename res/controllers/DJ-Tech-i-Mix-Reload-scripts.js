...
DJTechiMixReload.ledfunctions = function(ledname) {
    return function(value) {
        const eval = ledname;
        if (value != 0) { midi.sendShortMsg(0x90, DJTechiMixReload.buttons[ledname], 0x7f); } else { midi.sendShortMsg(0x90, DJTechiMixReload.buttons[ledname], 0x00); }
    };
};
DJTechiMixReload.allleadson = function() {
    for (const i in DJTechiMixReload.buttons) {
...
DJTechiMixReload.init = function(ID) {
    for (const key1 in DJTechiMixReload.buttons) {
        print("Button " + key1 + " -> MIDI " + DJTechiMixReload.buttons[key1]);
        for (const key2 in DJTechiMixReload.buttons) {
            if (key1 != key2) {
    ...
    for (let deck = 1; deck <= 2; deck++) {
        ...
        for (let hotcue = 1; hotcue <= 3; hotcue++) {
...
DJTechiMixReload.setfunction = function(channel, control, value, status, group) {
    ...
    } else { DJTechiMixReload.setbutton[group] = false; }
...
DJTechiMixReload.wheeltouch = function(channel, control, value, status, group) {
    ...
    print(group + group[8]);
    if (value == 0x00) { engine.scratchDisable(group[8]); } else { engine.scratchEnable(group[8], 148, 33+1/3.0, DJTechiMixReload.alpha, DJTechiMixReload.beta); }
};
