...
KANE_QuNeo.makeVar = function(element) {
    ...
    for (let i = 0; i < KANE_QuNeo.numDecks; i++) { array.push(element); }
    return array;
};
...
KANE_QuNeo.init = function(id) { // called when the device is opened & set up
    ...
    for (let deck = 1; deck <= KANE_QuNeo.numDecks; deck++) {
        const channelName = KANE_QuNeo.getChannelName(deck);
...
KANE_QuNeo.toggleJumpSync = function(deck) {
    ...
    const old = KANE_QuNeo.trackJumpSync[channel];
...
KANE_QuNeo.setJump = function(deck, direction) {
    const channel = deck - 1; // track channels start at 0 to properly reference arrays
    const current = KANE_QuNeo.trackJump[channel];
...
KANE_QuNeo.toggleLooping = function(deck) {
    const channel = deck - 1;
    const old = KANE_QuNeo.trackLooping[channel];
...
KANE_QuNeo.play = function(deck) {
    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    const playing = engine.getValue(channelName, "play");
    ...
    if (playing) {    // If currently playing
        ...
        if (KANE_QuNeo.trackJumpSync[channel]) { KANE_QuNeo.syncTrack(deck, "phase", 0); }
    }
    // update global value
    const hotcuePressed = KANE_QuNeo.hotcuePressed[channel];
...
KANE_QuNeo.togglePlayScratch = function(channel, control, value, status, group) {
    const old = KANE_QuNeo.playScratchToggle;
...
KANE_QuNeo.rotaryTouch = function(deck, value, status) {
    if ((status & 0xF0) == 0x90) {    // If note press on midi channel 1
        if (value == 0x7F) {   // if full velocity
            ...
            const alpha = 1.0/8, beta = alpha/32;
...
KANE_QuNeo.wheelTurn = function(deck, value) {
    let velocity = KANE_QuNeo.scratchSpeed;
...
KANE_QuNeo.toggleRecord = function(channel, control, value, status, group) {
    const old = KANE_QuNeo.recordToggle;
    ...
    for (let deck = 1; deck <= 2; deck++) {
        const channelName = KANE_QuNeo.getChannelName(deck);
        const samples = engine.getValue(channelName, "track_samples");
        const position = engine.getValue(channelName, "visual_playposition");
        const samplePosition = samples * position;
...
KANE_QuNeo.jumpLoop = function(deck, numBeats) {
    ...
    const samplerate = engine.getValue(channelName, "track_samplerate");
    const samples = engine.getValue(channelName, "track_samples");
    const bpm = engine.getValue(channelName, "file_bpm");
    const spb = samplerate * 60 * 2 / bpm; // samples per beat, not sure on the 2.

    // calculate the new position
    const oldPosition = engine.getValue(channelName, "visual_playposition");
    const direction = KANE_QuNeo.trackJump[channel];
    const beatsVector = numBeats * direction; // vectors have magnitude and direction
    const newPosition = oldPosition + (beatsVector*spb/samples);
    ...
    if (newPosition != oldPosition) {
        ...
        const wholeBeat = KANE_QuNeo.wholeBeat[channel];
        let newBeat = (wholeBeat + beatsVector);
        // if playing, we will hit the next beat so add 1
        if (engine.getValue(channelName, "play")) { newBeat += 1; }
        const totalBeats = KANE_QuNeo.totalBeats;

        if (newBeat < 1) // hand-made mod because javascript mod is wrong when < 0.
        { newBeat += totalBeats; } else if (newBeat > 16) { newBeat -= totalBeats; }
    ...
    // if neither jump nor loop, then we are in loop planning mode
        ...
        { KANE_QuNeo.loopNextJump[channel] = 0; } else // otherwise, proceed
...
KANE_QuNeo.syncTrack = function(deck, type, scheduleFlag) {
    const channel = deck - 1; // confusing, yes. channels start from 0.

    // verify other track is playing before syncing to avoid glitchy jumpsync loops
    const otherDeck = ((channel + 1) % 2) + 1;
    const otherTrackPlaying = engine.getValue("[Channel"+otherDeck+"]", "play");
    ...
    // KANE_QuNeo.syncLEDRed(deck);
        ...
        else { KANE_QuNeo.doSync(deck, type); } // else do it now
    }
};
...
KANE_QuNeo.deckMultiplyLoop = function(deck, factor) {
    ...
    let control;
    if (factor == 1/2) { control = "loop_halve"; } else if (factor == 2) { control = "loop_double"; } else { print("ERROR: wrong factor given for deckMultiplyLoop"); }
...
KANE_QuNeo.deckZoom = function(deck, value) {
    const channel = deck - 1; // track channels start at 0 to properly reference arrays
    const channelName = KANE_QuNeo.getChannelName(deck);
    const normalized = Math.ceil(6 * ((127 - value) / 127));
    // adjust zoom
...
KANE_QuNeo.doVisualNudge = function(deck, direction) {
    ...
    const newPosition = engine.getValue(channelName, "visual_playposition")
...
KANE_QuNeo.rateNudgeAll = function(callingDeck, direction) {
    const channelName = KANE_QuNeo.getChannelName(callingDeck);
    ...
    for (let deck = 1; deck <= KANE_QuNeo.numDecks; deck++) {
        if (deck != callingDeck) { KANE_QuNeo.doSync(deck, "tempo"); }
    }
    ...
    // slow to update.
    const rate = engine.getValue(channelName, "rate") + 0.16 * direction * 0.05;
    const tolerance = KANE_QuNeo.rateNudgeTolerance;
    if ((rate < 0 && rate >= -tolerance) || // if near 0% adjustment,
    (rate > 0 && rate < tolerance)) { KANE_QuNeo.cancelRateNudge(); } // cancel rate nudging
};

KANE_QuNeo.doRateNudge = function(deck, direction) {
    KANE_QuNeo.assertNudgeLEDs(); // update LEDs

    let control, channelName = KANE_QuNeo.getChannelName(deck);
    if (direction == 0) { return; } else if (direction == 1) // nudge tempo up
    { control = "rate_perm_up_small"; } else if (direction == -1) // nudge tempo down
...
KANE_QuNeo.makeCueDispatch = function(deck, cue, clearFlag, activateFlag) {
    let name = "deck"+deck+"Cue"+cue; // base name
...
KANE_QuNeo.makeCueDispatches = function(numDecks, numCues) {
    for (let deck = 1; deck <= numDecks; deck++) { // for each deck
        for (let cue = 1; cue <= numCues; cue++) { // and each cue
...
KANE_QuNeo.makeHorizontalArrowDispatch = function(deck, effect) {
    const name = "deck"+deck+effect; // base name
    // make function
    KANE_QuNeo[name] = function(channel, control, value, status, group) {
        const channelName = KANE_QuNeo.getChannelName(deck);
        const old = engine.getValue(channelName, effect);
        const toggled = (old + 1) % 2;
...
KANE_QuNeo.makeHorizontalArrowDispatches = function(numDecks) {
    const effects = ["keylock", "pfl", "slip_enabled", "flanger"];
    for (let deck = 1; deck <= numDecks; deck++) { // for each deck
        for (let i = 0; i < effects.length; i++) // and each effect
        // make dispatch
...
KANE_QuNeo.scrollPlaylist = function(channel, control, value, status, group) {
    ...
    { engine.setValue("[Playlist]", "SelectNextPlaylist", 1); } else if (value == 127) // if rotating counterclockwise
...
KANE_QuNeo.scrollTracklist = function(channel, control, value, status, group) {
    ...
    // otherwise, handle scrolling
    if (value == 1) // if rotating clockwise
    { engine.setValue("[Playlist]", "SelectNextTrack", 1); } else if (value == 127) // if rotating counterclockwise
...
KANE_QuNeo.resetBeat = function(deck) {
    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    const bpm = engine.getValue(channelName, "bpm");
...
KANE_QuNeo.quantizeCues = function(deck, LEDControl) {
    ...
    const trackSamples = engine.getValue(channelName, "track_samples");
    const position = engine.getValue(channelName, "visual_playposition")
    * trackSamples; // track position in samples
    ...
    for (let cue = 1; cue <= KANE_QuNeo.numHotcues; cue++) {
        const cuePosition = engine.getValue(channelName, "hotcue_"+cue+"_position");

        // if cue is enabled, reset it
        if (cuePosition >= 0) {
            const diffBeats = Math.round((cuePosition - position) / spb);
            const newPosition = position + (diffBeats * spb);
...
KANE_QuNeo.timeKeeper = function(deck, value) {
    ...
    // check for a next nearest hotcue, and update if we've passed it
        ...
        KANE_QuNeo.numNextHotcues[channel]) { KANE_QuNeo.assertHotcueLEDs(deck); }
...
KANE_QuNeo.getWholeBeat = function(deck, position, spb) {
    ...
    const diffBeat = Math.round((position - referencePosition)/spb);
    if (diffBeat > 0) { return (diffBeat % 16) + 1; } else { return (((diffBeat % 16)+ 16) % 16) + 1; }
};
...
KANE_QuNeo.handleBeat = function(deck) {
    ...
    const lastWholeBeat = KANE_QuNeo.wholeBeat[channel];

    // now see which beat this one is
    const value = engine.getValue(channelName, "visual_playposition");
    const samplerate = engine.getValue(channelName, "track_samplerate");
    const samples = engine.getValue(channelName, "track_samples");
    const bpm = engine.getValue(channelName, "file_bpm");
    const spb = samplerate * 60 * 2 / bpm; // samples per beat, not sure on the 2.
    const position = value * samples; // scale 0...1 to position in samples
    const diff = position - KANE_QuNeo.lastBeatPosition[channel];
    ...
    // this is not a consecutive beat:
        ...
        const numBeats = KANE_QuNeo.loopNextJump[channel];
        if (numBeats) {
            KANE_QuNeo.doLoop(deck, numBeats);
...
KANE_QuNeo.cancelScheduledBeats = function(deck) {
    let channel = deck - 1, i;
    const scheduledBeats = KANE_QuNeo.scheduledBeats[channel];
...
KANE_QuNeo.cancelTimers = function(timers) {
    for (i = 0; i < timers.length; i++) { engine.stopTimer(timers[i]); } // cancel each timer
};
...
KANE_QuNeo.scheduleBeat = function(deck, wholeBeat, direction) {
    ...
    const startOfCall = "KANE_QuNeo.deckBeatLEDs("+deck+","+wholeBeat+",";
...
KANE_QuNeo.oneToFiveKnob = function(value) {
    ...
    { return value * 127 / 2; } else // second half of the knob, 1-5
    { return 63.5 + (value - 1) * 63.5 / 4; }
};

KANE_QuNeo.getSliderControl = function(deck, side) {
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
...
KANE_QuNeo.delayedAssertion = function(functionName, oneShotFlag, delay) {
    ...
    if (delay == undefined) { delay = KANE_QuNeo.assertionDelayTimer; }
...
KANE_QuNeo.powerNeverOff = function(value, power) {
    ...
    if (value < KANE_QuNeo.minBrightness) { value = KANE_QuNeo.minBrightness; }
    return value;
};
...
KANE_QuNeo.getForwardValues = function(value) {
    return {
        squared: KANE_QuNeo.scaleToSlider(Math.pow(value, 2)),
...
KANE_QuNeo.getReverseValues = function(value) {
    ...
    { var zeroToFour1 = value / 63.5; } else // second half of the knob, 1...4
    { var zeroToFour1 = ((3 / 63.5) * value) - 2; }
    // determine 0...1 denormalized from 0...127 value
    const denormalized1 = value / 127;
    // determine -1...1
    const negOneToOne1 = value / 63.5 - 1;
    return {
        zeroToFour: zeroToFour1,
...
KANE_QuNeo.getChannelName = function(deck) {
    ...
    { return "[Master]"; } else if (deckType == "deck") // if dealing with actual decks
    { return "[Channel"+deck+"]"; } else if (deckType == "sampler") // if dealing with samplers
    { return "[Sampler"+(deck - 4)+"]"; }
};

KANE_QuNeo.getDeckType = function(deck) {
    if (deck == 0) // master deck
    { return "master"; } else if (deck < 5) // if dealing with actual decks
    { return "deck"; } else // if dealing with samplers
    { return "sampler"; }
};
...
KANE_QuNeo.deckBeatLEDs = function(deck, wholeBeat, quarter) {
    ...
    const channel = deck - 1; // yes very annoying
    const channelName = KANE_QuNeo.getChannelName(deck);
...
KANE_QuNeo.clearLastBeatLEDs = function(deck) {
    const channel = deck - 1;
    const off = KANE_QuNeo.activeBeatLEDs[channel]; // store for use
...
KANE_QuNeo.syncLEDRed = function(deck) {
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
    let on, off;
    if (LEDGroup == 1) { on = [0x03], off = [0x02]; } else { on = [0x0d], off = [0x0c]; }
    // emit updates
...
KANE_QuNeo.assertJumpLEDs = function(deck, numBeats) {
    const channel = deck - 1;
    let off = [], on = []; // arrays of LEDs to turn off or on;
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
    ...
    // emit updates
    const old = KANE_QuNeo.jumpLoopLEDs[channel];
...
KANE_QuNeo.playPressLEDs = function(deck) {
    let status = 0xB0, control, LEDGroup = KANE_QuNeo.getLEDGroup(deck);
...
KANE_QuNeo.circleLEDs = function(deck, value) {
    ...
    // on the vinyl.
    const time = value * engine.getValue(channelName, "duration");
    const revolutions = (time/revtime) - .25;

    // multiply the fractional part by the total number of LEDs in a rotary.
    const light = ((revolutions-(revolutions|0))*127)|0;
    ...
    // bwd the backward
    let status = 0xB0, control, fwd = 0x00, bwd = 0x7f;
...
KANE_QuNeo.assertLEDs = function(mode) {
    ...
    for (let deck = 0; deck <= KANE_QuNeo.numDecks; deck++) { KANE_QuNeo.triggerVuMeter(deck); }
...
KANE_QuNeo.closeMode = function(mode) {
    ...
    case 16:
        ...
        for (let i = 0; i < LEDs.length; i++) { KANE_QuNeo.LEDs(0x90, LEDs[i], 0x00); } // turn off all old LEDs
        break;
    }
};
...
KANE_QuNeo.assertNudgeLEDs = function() {
    ...
    { on.push(0x2e, 0x30), off.push(0x2f, 0x31); } else if (KANE_QuNeo.rateNudge == -1) // backward nudge LEDs on
    { on.push(0x2f, 0x31), off.push(0x2e, 0x30); } else if (KANE_QuNeo.rateNudge == 0) // no nudge LEDs on
...
KANE_QuNeo.assertHotcueActivateLEDs = function(deck) {
    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    let closest = [undefined, 2]; // next hotcue wrt current playpos as [control,pos]
    const position = engine.getValue(channelName, "visual_playposition"); // song position
    const trackSamples = engine.getValue(channelName, "track_samples");
    const on = [], LEDs = [[0x71, 0x73, 0x75, 0x77,
        0x61, 0x63, 0x65, 0x67,
        0x51, 0x53, 0x55, 0x57,
        0x41, 0x43, 0x45, 0x47],
    [0x79, 0x7b, 0x7d, 0x7f,
        ...
        if (!((cuePosition < 0 && cuePosition > -1e-7) ||
          cuePosition == -Infinity)) {
            ...
            if (KANE_QuNeo.mode == 13) {
                const red = LEDs[channel][cue - 1];

                // check to see if this hotcue is coming up
                const diff = cuePosition - position;
...
KANE_QuNeo.assertHotcueClearLEDs = function(deck) {
    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    const on = [], LEDs = [[0x79, 0x7b, 0x7d, 0x7f,
        0x69, 0x6b, 0x6d, 0x6f,
        0x59, 0x5b, 0x5d, 0x5f,
        0x49, 0x4b, 0x4d, 0x4f],
    [0x71, 0x73, 0x75, 0x77,
        ...
        0x41, 0x43, 0x45, 0x47]];
    for (let cue = 1; cue <= KANE_QuNeo.numHotcues; cue++) {
        if (engine.getValue(channelName, "hotcue_"+cue+"_enabled")) { on.push(LEDs[deck - 1][cue - 1]); } // add LED if cue enabled
    }
    // emit updates
...
KANE_QuNeo.clearHotcueLEDs = function(deck) {
    const channel = deck - 1;
    const LEDs = [[0x79, 0x7b, 0x7d, 0x7f,
        0x69, 0x6b, 0x6d, 0x6f,
        0x59, 0x5b, 0x5d, 0x5f,
        0x49, 0x4b, 0x4d, 0x4f],
    [0x71, 0x73, 0x75, 0x77,
        ...
        0x51, 0x53, 0x55, 0x57,
        0x41, 0x43, 0x45, 0x47]];
    for (let i = 0; i < LEDs[channel].length; i++) {  // for all LEDs of this deck
        const red = LEDs[channel][i];
        const green = red - 1;
...
KANE_QuNeo.assertHorizArrowLEDs = function(deck) {
    ...
    { KANE_QuNeo.assertDeckHorizArrowLEDs(deck); } else // dealing with sampler deck
...
KANE_QuNeo.assertDeckHorizArrowLEDs = function(deck) {
    ...
    if (mode != 9 && mode != 10) { // if not in sampler mode
        ...
        const controls = [["keylock", 0x24, 0x25],
            ["pfl", 0x26, 0x27]];
...
KANE_QuNeo.assertLoadLEDs = function() {
    const on = [], LEDs = [[0x08, 0x09], [0x0e, 0x0f], [undefined, undefined], [undefined, undefined], [0x00, 0x01], [0x02, 0x03], [0x04, 0x05], [0x06, 0x07]];
    // choose color based on whether or not the track is playing
    for (let channel = 0; channel < KANE_QuNeo.numDecks; channel++) {
...
KANE_QuNeo.assertKillLEDs = function(deck) {
    const channelName = KANE_QuNeo.getChannelName(deck);
    let on = [], off = [], i; // i for loop iteration
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
    const controls = [["filterHighKill", 0x37, 0x39, 0x77, 0x79],
...
KANE_QuNeo.assertJumpDirectionLEDs = function(deck) {
    const channel = deck - 1;
    let on = [], off = []; // controls for which LEDs to turn on and off
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
...
KANE_QuNeo.assertLoopingLED = function(deck) {
    const channel = deck - 1;
    let on = [], off = []; // arrays to control which LEDs to change
...
KANE_QuNeo.assertBeatCounterLEDs = function(deck) {
    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    const beat = KANE_QuNeo.wholeBeat[channel];
    const mode = KANE_QuNeo.mode;
    ...
    // first determine with which LEDs we are working
        ...
        if (engine.getValue(channelName, "duration") != 0) {
            ...
            if (beat < 5) { undefined; } // led off
            else if (beat < 9) { on.push(fours[0]); } // green on
            else if (beat < 13) { on.push(fours[1]); } // red on
            else { on = on.concat(fours); } // orange on
...
KANE_QuNeo.assertBeatLEDs = function(deck) {
    ...
    const mode = KANE_QuNeo.mode;
    if (!(mode == 13 || mode == 14 || mode == 15)) { return; }

    const channel = deck - 1;
    const channelName = KANE_QuNeo.getChannelName(deck);
    // arrays to control which LEDs to change
    let on, greens, reds, reloop, reloopOn = [];
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
    ...
    const controls = [["beatloop_1_enabled", [0x31], [0x3d], [0x71], [0x7d]],
    ...
    if (engine.getValue(channelName, "loop_enabled")) { reloopOn = reloop; } // light reloop LED if there is an enabled loop
    ...
    if (numBeats) { KANE_QuNeo.assertJumpLEDs(deck, numBeats); }
...
KANE_QuNeo.deckVuMeter = function(deck, value) {
    ...
    const values = KANE_QuNeo.getForwardValues(value);
    ...
    if (!KANE_QuNeo.rateNudge) {
        const controls = [[0x2e, 0x2f], [0x30, 0x31]];
    ...
    (mode == 13 || mode == 14 || mode == 15 || mode == 16)) {
        ...
        { level = values.squared; } else // else do volume
...
KANE_QuNeo.masterVuMeter = function(value) {
    ...
    let level = -1; // -1 for no playing decks
    for (let channel = 0; channel < KANE_QuNeo.numDecks; channel++) {
...
KANE_QuNeo.deckZoomLEDs = function(deck, value) {
    ...
    const zoom = ((value - 1) / 5) * 127;
    // determine which control we are manipulating
    const control = KANE_QuNeo.getSliderControl(deck, 0);
    // emit message
...
KANE_QuNeo.deckCursorLEDs = function(deck, position) {
    ...
    const normalized = position * 127;
    // determine which control we are manipulating
    const control = KANE_QuNeo.getSliderControl(deck, 1);
    // emit LED message
...
KANE_QuNeo.deckPregain = function(deck, value) {
    let gain, LEDGroup = KANE_QuNeo.getLEDGroup(deck);

    // determine gain
    if (value < 1) // first half of the knob, 0-1
    { gain = value * 127 / 2; } else // second half of the knob, 1-4
...
KANE_QuNeo.deckRate = function(deck, value) {
    const LEDGroup = KANE_QuNeo.getLEDGroup(deck);
    // determine rate
    const rate = (value * 63.5) + 63.5; // scaled for -1...1 to map to 0...127
    // now determine control
    const control = KANE_QuNeo.getSliderControl(deck, 1);
    // emit message
...
KANE_QuNeo.deckHighs = function(deck, value) {
    ...
    // determine level
    if (value < 1) // first half of the knob, 0-1
    { level = value * 127 / 2; } else // second half of the knob, 1-4
...
KANE_QuNeo.deck1LeftVol = function(value) {
    const vol = Math.pow(value, 3) * 127;
