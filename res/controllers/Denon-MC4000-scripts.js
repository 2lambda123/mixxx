...
MC4000.init = function() {
    ...
    const byteArray = [0xF0, 0x00, 0x02, 0x0B, 0x7F, 0x01, 0x60, 0x00, 0x04, 0x04, 0x01, 0x00, 0x00, 0xF7];
...
MC4000.Deck = function(channel) {
    ...
    this.jogWheel = function(channel, control, value, status, group) {
        const numTicks = (value < 0x40) ? value: (value - 0x80);
        if (engine.isScratching(this.engineChannel)) {
            engine.scratchTick(this.engineChannel, numTicks);
        } else {
            const jogDelta = numTicks/MC4000.jogWheelTicksPerRevolution*MC4000.jogParams.sensitivity;
            const jogAbsolute = jogDelta + engine.getValue(group, "jog");
            const limit = MC4000.jogParams.maxJogValue;
...
MC4000.fxWetDry = function(midichan, control, value, status, group) {
    const numTicks = (value < 0x40) ? value: (value - 0x80);
    const newVal = engine.getValue(group, "mix") + numTicks/64.0*2;
...
MC4000.getNextRateRange = function(currRateRange) {
    for (let i = 0; i < MC4000.rateRanges.length; i++) {
