...
DNHS5500.rateDisplay = function(value, group) {
    const channelmidi = DNHS5500.channelForGroup(group);
    const rateDir = engine.getValue(group, "rate_dir");
    const raterange = engine.getValue(group, "rateRange");
    const rate = engine.getValue(group, "rate");
    const slider_rate = ((rate * raterange) * 100) * rateDir;
    const rate_abs = Math.abs(slider_rate);
    const rate_dec = Math.floor(rate_abs);
    const rate_frac = Math.round((rate_abs - rate_dec) * 100);
    ...
    const bpm_dec = Math.floor(bpm / 10);
    const bpm_frac = Math.round((bpm - (bpm_dec * 10)) * 10);
...
DNHS5500.playPositionChanged = function(value, group) {
    ...
    const reversed = engine.getValue(group, "reverse");
    if (reversed) {
    ...
    const pos_total_secs = value * duration;
    const pos_minutes = Math.floor(pos_total_secs / 60);
    const pos_secs = pos_total_secs % 60;
    const pos_frac = Math.round((pos_total_secs - Math.floor(pos_total_secs)) * 100);
...
DNHS5500.wheelTouch = function(channel, control, value, status) {
    const decknum = DNHS5500.deckForChannel(channel);
    if (value == 0x40) {
...
DNHS5500.scratchEnable = function(deck, ramp) {
    ...
    const damp_value = 32;
    const alpha = 1.0 / damp_value;
    const beta = alpha / damp_value;
...
DNHS5500.wheelTurn = function(channel, control, value, status) {
    ...
    const deckNumber = DNHS5500.deckForChannel(channel);
    const group = DNHS5500.groupForChannel(channel);

    // Velocity values.
    let velocity = value - 0x40;
    ...
    const currentlyPlaying = engine.getValue(group, "play");
    if (currentlyPlaying) {
...
DNHS5500.playButton = function(channel, control, value, status) {
    ...
    const channelname = DNHS5500.groupForChannel(channel);
    const currentlyPlaying = engine.getValue(channelname, "play");
    // Toggle it.
    if (currentlyPlaying) {
...
DNHS5500.toggleLightLayer1 = function(group, light, status) {
    const channel = DNHS5500.channelForGroup(group);
    if (status > 0) {
