...
DenonDNSC2000.init = function(id) {
    ...
    const leds = [0x11, 0x13, 0x15, 0x17, 0x19, 0x1B, 0x1D, 0x20, /* cues */
    ...
    for (let index = 0, count = leds.length; index < count; index ++) {
...
DenonDNSC2000.getDeckByGroup = function(group) {
    for (let index = 0, count = decks.length; index < count; index++) {
        if (decks[index].group == group) { return decks[index]; }
    }
    return null;
};
...
DenonDNSC2000.newValue = function(currentVal, min, max, increment, ticksCount) {
    const interval = (max - min) / 24;
    let newVal = 0;
    if (increment) { newVal = Math.min(max, currentVal + interval); } else { newVal = Math.max(min, currentVal - interval); }
    return newVal;
};
...
DenonDNSC2000.selectTrack = function(midino, control, value, status, group) {
    if (value == 0x00) { engine.setValue("[Playlist]", "SelectNextTrack", 1); } else { engine.setValue("[Playlist]", "SelectPrevTrack", 1); }
};
...
DenonDNSC2000.loopOrHotcues = function(midino, control, value, status, group) {
    const deck = DenonDNSC2000.getDeckByGroup(group);
    if ((status & 0xF0) == 0x80) { deck.controlPressed = -1; } else {
...
DenonDNSC2000.hotcue = function(cueIndex, group, value, shift) {
    if (!shift) {
        ...
        if (engine.getValue(group, "hotcue_" + cueIndex + "_enabled") == 0) {
            const samplesPerBeat = DenonDNSC2000.samplesPerBeat(group);
            let positionInBeats = (engine.getValue(group, "playposition") * engine.getValue(group, "track_samples")) / samplesPerBeat;
            if ((positionInBeats - Math.floor(positionInBeats)) > 0.5) { positionInBeats = Math.floor(0.5 + positionInBeats) * samplesPerBeat; } else { positionInBeats = Math.floor(positionInBeats) * samplesPerBeat; }
        ...
        } else { engine.setValue(group, "hotcue_" + cueIndex + "_clear", 1); }
    }
...
DenonDNSC2000.reloop = function(group, value, shift) {
    let loopInPosition = engine.getValue(group, "loop_start_position");
    ...
    if (loopInPosition != -1 && loopOutPosition!=-1) {
        ...
        if (shiftPressed) {
            const loopInPositionInBeats = loopInPosition / samplesPerBeat;
            if ((loopInPositionInBeats - Math.floor(loopInPositionInBeats)) > 0.5) { loopInPosition = Math.floor(0.5 + loopInPositionInBeats) * samplesPerBeat; } else { loopInPosition = Math.floor(loopInPositionInBeats) * samplesPerBeat; }
        } else { loopInPosition = engine.getValue(group, "playposition") * engine.getValue(group, "track_samples"); }
...
DenonDNSC2000.resizeLoop = function(midino, control, value, status, group) {
    let increment = true;
    if (value == 0x7F) { increment = false; }
    const loopInPosition = engine.getValue(group, "loop_start_position");
    const loopOutPosition = engine.getValue(group, "loop_end_position");
    if (loopInPosition != -1 && loopOutPosition!= -1) {
        const samplesPerBeat = DenonDNSC2000.samplesPerBeat(group);
        const deltaSamples = loopOutPosition - loopInPosition;
        let newLoopOutPosition = loopOutPosition;
        const loopLengthes = [
...
DenonDNSC2000.moveLoopLeft = function(midino, control, value, status, group) {
    const samplesPerBeat = Math.floor(0.5 + DenonDNSC2000.samplesPerBeat(group));
    let loopInPosition = engine.getValue(group, "loop_start_position");
    let loopOutPosition = engine.getValue(group, "loop_end_position");
...
DenonDNSC2000.play = function(midino, control, value, status, group) {
    if (shiftPressed) {
        if ((status & 0xF0) == 0x90 && engine.getValue(group, "play") == 1) { engine.setValue(group, "reverse", 1); } else
            if ((status & 0xF0) == 0x80 && engine.getValue(group, "play") == 1) { engine.setValue(group, "reverse", 0); }
    } else {
        if ((status & 0xF0) == 0x90) {
...
DenonDNSC2000.cue = function(midino, control, value, status, group) {
    const ledChannel = DenonDNSC2000.getLedChannelByGroup(group);
    if ((status & 0xF0) == 0x90) {
...
DenonDNSC2000.pitchBend = function(midino, control, value, status, group) {
    switch (control) {
    case 0x0D:
        if ((status & 0xF0) == 0x90) { engine.setValue(group, "rate_temp_down", 1); } else { engine.setValue(group, "rate_temp_down", 0); }
        break;
    case 0x0C:
        if ((status & 0xF0) == 0x90) { engine.setValue(group, "rate_temp_up", 1); } else { engine.setValue(group, "rate_temp_up", 0); }
        break;
    }
};
...
DenonDNSC2000.jog = function(midino, control, value, status, group) {
    if (shiftPressed) {
        ...
        const deck = DenonDNSC2000.getDeckByGroup(group);
        if (!deck.scratchMode) { deck.picthJog(value); } else { deck.scratchJog(value); }
    }
};
...
DenonDNSC2000.handleLoopAndHotcuesLeds = function(group) {
    ...
    const cueLeds = [0x11, 0x13, 0x15, 0x17, 0x19, 0x1B, 0x1D, 0x20];
    for (let index = 0, count = cueLeds.length; index < count; index ++) {
...
DenonDNSC2000.handleFlangerLed = function(group) {
    const flangerOn = (engine.getValue(group, "flanger") == 1);
...
DenonDNSC2000.getLedChannelByGroup = function(group) {
    if (group == "[Channel1]") { return 0xB0; } else
        if (group == "[Channel2]") { return 0xB1; } else
            if (group == "[Channel3]") { return 0xB2; } else
                if (group == "[Channel4]") { return 0xB3; }
};
