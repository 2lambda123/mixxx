...
Wiimote.init = function(id) {
    ...
    const controller = Wiimote.controller;
    ...
    controller.startAutoRepeatTimer = function(timer_id, interval) {
        if (controller.timers[timer_id]) { return; }
...
Wiimote.selectInputMode = function(mode) {
    ...
    const packet = Wiimote.controller.getInputPacket(mode);

    const callbacks = Wiimote[mode+"_Callbacks"];
...
Wiimote.updateReportMode = function(code, continuous) {
    const controller = Wiimote.controller;
    const packet = Wiimote.controller.getOutputPacket("setreportmode");
    if (packet==undefined) {
        HIDDebug("No output packet "+packetname+" defined");
        return;
    }
    if (code!=undefined) { Wiimote.reportMode = code; }
    if (continuous!=undefined) { Wiimote.continuosReporting = !!(continuous); }
...
Wiimote.buttons_Callbacks = function(packetname) {
    ...
    const controller = Wiimote.controller;
    const packet = Wiimote.controller.getInputPacket(packetname);
    if (packet==undefined) {
...
Wiimote.toggleLED = function(deck) {
    const controller = Wiimote.controller;
    const packet = Wiimote.controller.getOutputPacket("feedback");
    const active_led = "led_" + deck;
    controller.setOutput("state", "led_1", 0);
    controller.setOutput("state", "led_2", 0);
    controller.setOutput("state", "led_3", 0);
    controller.setOutput("state", "led_4", 0);
...
Wiimote.rumble = function(rumble, milliseconds) {
    const controller = Wiimote.controller;
    const packet = Wiimote.controller.getOutputPacket("feedback");
    if (rumble!=undefined) { rumble = (rumble) ? 1 : 0; }
...
Wiimote.seek_loop = function(field) {
    const controller = Wiimote.controller;
    let value;
    const group = controller.resolveDeckGroup(controller.activeDeck);
    if (group==undefined) { return; }
    if (field.value==controller.buttonStates.released) { value = false; } else { value = true; }
    if (controller.modifiers.get("shift")) {
        if (field.name=="arrow_left") { engine.setValue(group, "back", value); }
        if (field.name=="arrow_right") { engine.setValue(group, "fwd", value); }
    } else {
        // Reset back/gwd states (can be left on if you release shift)
        if (engine.getValue(group, "back")) { engine.setValue(group, "back", false); }
        if (engine.getValue(group, "fwd")) { engine.setValue(group, "fwd", false); }
        if (field.name=="arrow_left") { engine.setValue(group, "loop_in", value); }
        if (field.name=="arrow_right") {
            ...
            } else { engine.setValue(group, "loop_out", value); }
        }
    }
};
    ...
    if (!controller.modifiers.get("shift"))  {
        ...
        if (field.name=="arrow_up") {
            callback = function(field) {
                const group = Wiimote.controller.resolveDeckGroup(Wiimote.controller.activeDeck);
...
Wiimote.hotcue = function(field) {
    ...
    if (controller.modifiers.get("shift")) {
        if (field.value==controller.buttonStates.released) { return; }
        if (field.name=="button_1") { engine.setValue(group, "hotcue_1_clear", true); }
        if (field.name=="button_2") { engine.setValue(group, "hotcue_2_clear", true); }
    } else {
        if (field.value==controller.buttonStates.released) { return; }
        if (field.name=="button_1") { engine.setValue(group, "hotcue_1_activate", true); }
        if (field.name=="button_2") { engine.setValue(group, "hotcue_2_activate", true); }
    }
};
