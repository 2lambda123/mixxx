...
VMS2.hotCues = {
    0x12: 1, 0x13: 2, 0x14: 3, 0x15: 4, 0x17: 5, 0x18: 6,
...
VMS2.init = function(id) {    // called when the MIDI device is opened & set up
    ...
    for (let i=12; i<=77; i++) { midi.sendShortMsg(0x80, i, 0x00); }  // Extinquish all LEDs
...
VMS2.hotCue = function(channel, control, value, status, group) {
    const deck = VMS2.getDeck(group);
    const hotCue = VMS2.hotCues[control];
    if (value === ButtonState.pressed) {
...
VMS2.trackSelect = function(channel, control, value, status, group) {
    ...
    if (!isNaN(VMS2.trackSelect.last_value)) {
        let direction = value - VMS2.trackSelect.last_value;
