...
KAOSSDJ.init = function(id, debugging) {
    ...
    for (let led = 0x00; led <= 0xFF; led++) {
        for (let i = 0; i < ledChannels.length; i++) {
...
KAOSSDJ.wheelTouch = function(channel, control, value, status, group) {
    ...
    const deck_playing = engine.getValue("[Channel" + deckNumber + "]", "play_indicator");
