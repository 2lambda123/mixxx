...
BehringerBCD3000.init = function(id) { // called when the device is opened & set up
    ...
    if (BehringerBCD3000.UseAcceleration) { midi.sendShortMsg(0xB0, 0x63, 0x7F); } else { midi.sendShortMsg(0xB0, 0x63, 0x0); }
};
...
BehringerBCD3000.shutdown = function() {
    ...
    if (!BehringerBCD3000.UseAcceleration) { midi.sendShortMsg(0xB0, 0x63, 0x7F); }
};
...
BehringerBCD3000.getDeck = function(group) {
    if (group == "[Channel1]") { return 0; } else if (group == "[Channel2]") { return 1; }
...
BehringerBCD3000.jogWheel = function(channel, control, value, status, group) {
    ...
    if (BehringerBCD3000.escratch[deck]) {
        ...
        if (BehringerBCD3000.debug) { print(group + " scratch tick : " + scratchValue); }
        ...
        if (BehringerBCD3000.debug) { print(group + " pitching jog adjust : " + jogValue); }
...
BehringerBCD3000.scratchButton = function(channel, control, value, status, group) {
    ...
    if (BehringerBCD3000.debug) { print(group + " scratch enabled :" + BehringerBCD3000.escratch[deck]); }

    if (BehringerBCD3000.escratch[deck]) {
        // Turn on the scratch light
        if (!deck) { midi.sendShortMsg(0xB0, 0x13, 0x7F); } else { midi.sendShortMsg(0xB0, 0x0B, 0x7F); }
        ...
        if (!deck) { midi.sendShortMsg(0xB0, 0x13, 0x00); } else { midi.sendShortMsg(0xB0, 0x0B, 0x00); }
...
BehringerBCD3000.loop = function(channel, control, value, status, group) {
    if (value) { action = "loop_in"; } else { action = "loop_out"; }

    if (BehringerBCD3000.debug) { print(group + " " + action); }
