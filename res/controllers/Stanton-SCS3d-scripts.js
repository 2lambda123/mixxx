...
StantonSCS3d.buttonLEDs = {
    0x48: 0x62, 0x4A: 0x61, 0x4C: 0x60, 0x4e: 0x5f, 0x4f: 0x67, 0x51: 0x68, 0x53: 0x69, 0x55: 0x6a,
    0x56: 0x64, 0x58: 0x65, 0x5A: 0x6C, 0x5C: 0x5D
}; // Maps surface buttons to corresponding circle LEDs
StantonSCS3d.mode_store = {
    "[Channel1]": "vinyl", "[Channel2]": "vinyl", // Set vinyl mode on all decks
...
StantonSCS3d.loopButtons = {
    0x4E: 0.125, 0x4C: 0.25, 0x4A: 0.5, 0x48: 1,
    0x4F: 2, 0x51: 4, 0x53: 8, 0x55: 16
};
// Pitch values for key change mode
StantonSCS3d.pitchPoints = {
    1: {
        0x48: -0.1998, 0x4A: -0.1665, 0x4C: -0.1332, 0x4E: -0.0999, 0x56: -0.0666, 0x58: -0.0333,
        0x5A: 0.0333, 0x5C: 0.0666, 0x4F: 0.0999, 0x51: 0.1332, 0x53: 0.1665, 0x55: 0.1998
    }, // 3.33% increments
    2: {
        0x48: -0.5, 0x4A: -0.4043, 0x4C: -0.2905, 0x4E: -0.1567, 0x56: -0.1058, 0x58: -0.0548,
        0x5A: 0.06, 0x5C: 0.12, 0x4F: 0.181, 0x51: 0.416, 0x53: 0.688, 0x55: 1.0
    },  // Key changes
    3: {
        0x48: -0.4370, 0x4A: -0.3677, 0x4C: -0.3320, 0x4E: -0.2495, 0x56: -0.1567, 0x58: -0.0548,
        0x5A: 0.12, 0x5C: 0.263, 0x4F: 0.338, 0x51: 0.506, 0x53: 0.688, 0x55: 0.895
    }
};  // Notes
// Multiple banks of multiple cue points:
StantonSCS3d.hotCues = {
    1: {
        0x48: 1, 0x4A: 2, 0x4C: 3, 0x4E: 4, 0x4F: 5, 0x51: 6,
        0x53: 7, 0x55: 8, 0x56: 9, 0x58: 10, 0x5A: 11, 0x5C: 12
    },
    2: {
        0x48: 13, 0x4A: 14, 0x4C: 15, 0x4E: 16, 0x4F: 17, 0x51: 18,
        0x53: 19, 0x55: 20, 0x56: 21, 0x58: 22, 0x5A: 23, 0x5C: 24
    },
    3: {
        0x48: 25, 0x4A: 26, 0x4C: 27, 0x4E: 28, 0x4F: 29, 0x51: 30,
...
//     midi.sendSysexMsg(StantonSCS3d.sysex.concat([0x02, StantonSCS3d.channel, 0xF7]),7);
    ...
    const CC = 0xB0 + StantonSCS3d.channel;
    const No = 0x90 + StantonSCS3d.channel;
...
StantonSCS3d.statusResponse = function(data, length) {
    const statusResponsePreamble=[0xF0, 0x7E, 0, 6, 2, 0, 1, 0x60, 0x2c, 1, 1, 0];
...
StantonSCS3d.stopTimers = function() {
    for (let i=0; i<StantonSCS3d.timer.length; i++) {
...
StantonSCS3d.connectSurfaceSignals = function(channel, disconnect) {
    ...
    const signalList = StantonSCS3d.modeSignals[StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"]];
    for (i=0; i<signalList.length; i++) {
        let group = signalList[i][0];
        ...
        if (StantonSCS3d.debug) {
            if (disconnect) { print("StantonSCS3d: "+group+","+item+" disconnected from "+signalList[i][2]); } else { print("StantonSCS3d: "+group+","+item+" connected to "+signalList[i][2]); }
        }
    }
...
StantonSCS3d.connectDeckSignals = function(channel, disconnect, list) {
    ...
    for (i=0; i<signalList.length; i++) {
        ...
        if (!disconnect) {
            const currentValue = engine.getValue(group, name);
            switch (name) {
            case "play_indicator":
        ...
        if (StantonSCS3d.debug) {
            if (disconnect) { print("StantonSCS3d: "+group+","+signalList[i][1]+" disconnected from "+signalList[i][2]); } else { print("StantonSCS3d: "+group+","+signalList[i][1]+" connected to "+signalList[i][2]); }
        }
    }
    // If disconnecting signals, darken the corresponding LEDs
    if (disconnect) {
        const CC = 0xB0 + channel;
        const No = 0x90 + channel;
        switch (list) {
        case "common":
...
StantonSCS3d.circleLEDsColor = function(channel, color, side) {
    ...
    let start;
    let end;
    if (side=="left") { start = 0x5e; end = 0x63; } else { start = 0x66; end = 0x6b; }
...
StantonSCS3d.pitch = function(channel, control, value) {   // Lower the sensitivity of the pitch slider
    ...
    const currentValue = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rate");
    let newValue;
    const modDeck = StantonSCS3d.modifier[StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"]]==1;
    const swap = StantonSCS3d.finePitchDefault;
    if ((modDeck && !swap) || (!modDeck && swap)) {
        // Fine pitch adjust
        const pitchRange = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rateRange");
...
StantonSCS3d.pitchTouch = function(channel, control, value, status) {
    if ((status & 0xF0) == 0x80) {   // If button up
        ...
        if (engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rate_temp_down") != 0) { engine.setValue("[Channel"+StantonSCS3d.deck+"]", "rate_temp_down", 0); }
        if (engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rate_temp_up") != 0) { engine.setValue("[Channel"+StantonSCS3d.deck+"]", "rate_temp_up", 0); }
    }
};

StantonSCS3d.gain = function(channel, control, value) {
    const currentMode = StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"];
...
StantonSCS3d.gainRelative = function(channel, control, value) {
    ...
    if (StantonSCS3d.modifier[currentMode]==1 &&
        ...
        const add = StantonSCS3d.BoostCut(9, newValue, 0.0, 1.0, 4.0, 4, 4);
        const byte1 = 0xB0 + channel;
...
StantonSCS3d.playButton = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        const currentlyPlaying = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "play");
...
StantonSCS3d.cueButton = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) != 0x80) {    // If button down
        ...
        if (StantonSCS3d.modifier.vinyl || StantonSCS3d.modifier.vinyl2) {
            ...
            if (StantonSCS3d.scratchncue[StantonSCS3d.deck]) { StantonSCS3d.scratchncue[StantonSCS3d.deck]=false; } else { StantonSCS3d.scratchncue[StantonSCS3d.deck]=true; }
...
StantonSCS3d.syncButton = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (currentMode == "deck" && (!StantonSCS3d.singleDeck || StantonSCS3d.modifier.deck==1)) {
            ...
            const curval = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "quantize");
...
StantonSCS3d.B11 = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (StantonSCS3d.modifier[currentMode]==1 &&
            ...
            // Update the LEDs
            const add = StantonSCS3d.BoostCut(9, 1.0, 0.0, 1.0, 4.0, 5, 4);
...
StantonSCS3d.B12 = function(channel, control, value, status, group) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        let modeIndex = currentMode.charAt(currentMode.length-1);
...
StantonSCS3d.B13 = function(channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (currentMode == "vinyl3") { midi.sendShortMsg(byte1, control, 0x01); } // Make button red
    } else {
        StantonSCS3d.modifier.B13=0;   // Clear button modifier flag
        if (currentMode == "vinyl3") { midi.sendShortMsg(byte1, control, 0x02); } // Make button blue
    }
    switch (currentMode) {
    case "vinyl3":
        if ((status & 0xF0) == 0x90) {    // If button down
...
StantonSCS3d.modeButton = function(channel, control, status, modeName) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (currentMode == modeName) {
            ...
            const add = StantonSCS3d.BoostCut(9, engine.getValue("[Channel"+StantonSCS3d.deck+"]", "pregain"), 0.0, 1.0, 4.0, 5, 4);
            midi.sendShortMsg(0xB0+channel, 0x07, 0x15+add);
    ...
    case "loop":
        ...
        for (i=0; i<redButtonLEDs.length; i++) { midi.sendShortMsg(byte1, redButtonLEDs[i], 0x40); } // Set them to black
        break;
    case "loop2":
    case "loop3":
    case "trig":
    ...
    case "trig3":
        ...
        for (i=0; i<redButtonLEDs.length; i++) { midi.sendShortMsg(byte1, redButtonLEDs[i], 0x40); } // Set them to black
        for (i=0x56; i<=0x5c; i++) { midi.sendShortMsg(byte1, i, 0x40); } // Set center slider to black
        break;
    case "deck":
    ...
    case "loop":
        var redButtonLEDs = [0x48, 0x4a, 0x4c, 0x4e, 0x4f, 0x51, 0x53, 0x55, 0x56, 0x58, 0x59, 0x5A, 0x5C];
        for (i=0; i<redButtonLEDs.length; i++) { midi.sendShortMsg(byte1, redButtonLEDs[i], 0x41); } // Set them to red dim
        break;
    case "loop2":
    case "loop3":
    case "trig":
...
StantonSCS3d.FX = function(channel, control, value, status) {
    ...
    switch (currentMode) {
    case "fx":
        if ((status & 0xF0) == 0x80) { mode = "fx2"; } else { mode = "fx"; }
        break;
    case "fx2":
        if ((status & 0xF0) == 0x80) { mode = "fx3"; } else { mode = "fx2"; }
        break;
    case "fx3":
        if ((status & 0xF0) == 0x80) { mode = "fx"; } else { mode = "fx3"; }
        break;
    default: mode = "fx";
    }
...
StantonSCS3d.Loop = function(channel, control, value, status) {
    ...
    switch (currentMode) {
    case "loop":
        if ((status & 0xF0) == 0x80) { mode = "loop2"; } else { mode = "loop"; }
        break;
    case "loop2":
        if ((status & 0xF0) == 0x80) { mode = "loop3"; } else { mode = "loop2"; }
        break;
    case "loop3":
        if ((status & 0xF0) == 0x80) { mode = "loop"; } else { mode = "loop3"; }
        break;
    default: mode = "loop";
    }
    StantonSCS3d.modeButton(channel, control, status, mode);
...
StantonSCS3d.Trig = function(channel, control, value, status) {
    ...
    switch (currentMode) {
    case "trig":
        if ((status & 0xF0) == 0x80) { mode = "trig2"; } else { mode = "trig"; }
        break;
    case "trig2":
        if ((status & 0xF0) == 0x80) { mode = "trig3"; } else { mode = "trig2"; }
        break;
    case "trig3":
        if ((status & 0xF0) == 0x80) { mode = "trig"; } else { mode = "trig3"; }
        break;
    default: mode = "trig";
    }
    StantonSCS3d.modeButton(channel, control, status, mode);
...
StantonSCS3d.Vinyl = function(channel, control, value, status) {
    ...
    switch (currentMode) {
    case "vinyl":
        if ((status & 0xF0) == 0x80) { mode = "vinyl2"; } else { mode = "vinyl"; }
        break;
    case "vinyl2":
        if ((status & 0xF0) == 0x80) { mode = "vinyl3"; } else { mode = "vinyl2"; }
        break;
    case "vinyl3":
        if ((status & 0xF0) == 0x80) { mode = "vinyl"; } else { mode = "vinyl3"; }
        break;
    default: mode = "vinyl";
    }
    StantonSCS3d.modeButton(channel, control, status, mode);
...
StantonSCS3d.DeckButton = function(channel, control, value, status) {
    if (StantonSCS3d.singleDeck) { StantonSCS3d.modeButton(channel, control, status, "deck"); } else { StantonSCS3d.DeckChangeP1(channel, control, value, status); }
};

StantonSCS3d.deckChangeFlash = function(channel, value, targetSide) {
    const led = {"left": 0x71, "right": 0x72};
    const byte1 = 0x90 + channel;
...
StantonSCS3d.DeckChangeP1 = function(channel, control, value, status) {
    ...
    if ((new Date() - StantonSCS3d.modifier.deckTime)>=StantonSCS3d.deckChangeWait) {
        ...
        if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"].substring(0, 4) == "trig" ||
            StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"].substring(0, 4) == "loop") { for (i=0x48; i<=0x5c; i++) { midi.sendShortMsg(byte1, i, 0x40); } } // Set surface LEDs to black
        StantonSCS3d.deck++;
...
StantonSCS3d.DeckChangeP2 = function(channel, value) {
    ...
    case "fx3":
        if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] == "none") { StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] = "fx2"; }    // force correct change
    case "fx":
    ...
    case "loop3":
        if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] == "none") { StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] = "loop2"; }    // force correct change
    case "loop":
    ...
    case "trig3":
        if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] == "none") { StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] = "trig2"; }    // force correct change
    case "trig":
    ...
    case "vinyl3":
        if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] == "none") { StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"] = "vinyl2"; }   // force correct change
    case "vinyl":
...
StantonSCS3d.S4absolute = function(channel, control, value) {
    ...
    case "loop3":
        ...
        //                 break;
        //             }
        if (StantonSCS3d.pitchPoints[index][button] == -0.1) { StantonSCS3d.pitchPoints[index][button] = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rate"); } else {
...
StantonSCS3d.S5relative = function(channel, control, value) {
    ...
    if (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"]=="deck") {
        let newValue = engine.getValue("[Master]", "headVolume")+(value-64)/128;
        if (newValue<0.0) { newValue=0.0; }
        if (newValue>5.0) { newValue=5.0; }
...
StantonSCS3d.S4buttonLights = function(channel, light, button) {     // Turn on/off button lights for multiple buttons
    ...
    if (StantonSCS3d.markHotCues == "red") { buttonLight = StantonSCS3d.buttonLEDs[button]; } else { buttonLight = button; }
    midi.sendShortMsg(byte1, buttonLight, color);

    const currentMode = StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"];
    let index = currentMode.charAt(currentMode.length-1);
    if (index != "2" && index != "3") { index = "1"; }
};
...
StantonSCS3d.SurfaceButton = function(channel, control, value, status) {
    ...
    let buttonLight;
    if (StantonSCS3d.markHotCues == "red") { buttonLight = StantonSCS3d.buttonLEDs[control]; } else { buttonLight = control; }
    ...
    if ((status & 0xF0) != 0x80) {    // If button down
        ...
        case "loop3":
            ...
            //                     break;
            //                 }
            if (StantonSCS3d.pitchPoints[index][control] == -0.1) { StantonSCS3d.pitchPoints[index][control] = engine.getValue("[Channel"+StantonSCS3d.deck+"]", "rate"); } else {
                // Need 100% range for values to be correct
                engine.setValue("[Channel"+StantonSCS3d.deck+"]", "rateRange", 1.0);
...
StantonSCS3d.buttonLED = function(value, note, on, off) {
    const byte1 = 0x90 + StantonSCS3d.channel;
    if (value>0) { midi.sendShortMsg(byte1, note, on); } else { midi.sendShortMsg(byte1, note, off); }
};
...
StantonSCS3d.BsLED = function(value, button, activate) {
    ...
    const byte1 = 0x90 + StantonSCS3d.channel;
    if (value != 0) { midi.sendShortMsg(byte1, light, 0x01); } else { midi.sendShortMsg(byte1, light, 0x00); }
};
...
StantonSCS3d.ActiveLoop = function(value) {
    const timerName = "loop"+StantonSCS3d.deck;
    if (value>0) {
...
StantonSCS3d.BoostCut = function(numberLights, value, low, mid, high, lowMidSteps, midHighSteps) {
    let LEDs = 0;
    const lowMidInterval = (mid-low)/(lowMidSteps*2);     // Half the actual interval so the LEDs light in the middle of the interval
    const midHighInterval = (high-mid)/(midHighSteps*2);  // Half the actual interval so the LEDs light in the middle of the interval
...
StantonSCS3d.Peak7 = function(value, low, high) {
    let LEDs = 0;
    const halfInterval = ((high-low)/6)/2;
    value=value.toFixed(4);
    if (value>low) { LEDs++; }
...
StantonSCS3d.headMixLEDs = function(value) {
    const add = StantonSCS3d.BoostCut(7, value, -1.0, 0.0, 1.0, 3, 3);
    const byte1 = 0xB0 + StantonSCS3d.channel;
    midi.sendShortMsg(byte1, 0x0C, 0x15+add);
};

StantonSCS3d.headVolLEDs = function(value) {
    const add = StantonSCS3d.BoostCut(7, value, 0, 1, 5, 3, 3);
    const byte1 = 0xB0 + StantonSCS3d.channel;
...
StantonSCS3d.EQLEDs = function(value, control, item) {
    const group = "[EqualizerRack1_[Channel"+StantonSCS3d.deck+"]_Effect1]";
    //     var add = StantonSCS3d.BoostCut(7,value, 0, 1, 4, 3, 3);
    const add = StantonSCS3d.BoostCut(7,
...
StantonSCS3d.FXSliderLEDs = function(value, control, item) {
    ...
    let index = currentMode.charAt(currentMode.length-1);
    if (index != "2" && index != "3") { index = "1"; }
    const group = "[EffectRack1_EffectUnit"+index+"_Effect1]";
    const add = StantonSCS3d.Peak7(engine.getParameterForValue(group, item, value),
        0.0, 1.0);
    const byte1 = 0xB0 + StantonSCS3d.channel;
...
StantonSCS3d.VUMeterLEDs = function(value) {
    ...
    const add = StantonSCS3d.Peak7(value, 0, 1);
    const byte1 = 0xB0 + StantonSCS3d.channel;
    if (StantonSCS3d.deck % 2 == 0) { midi.sendShortMsg(byte1, 0x0E, 0x28+add); } else { midi.sendShortMsg(byte1, 0x0C, 0x28+add); }
};
...
StantonSCS3d.MasterVolumeLEDs = function(value) {
    let LEDs = 0;
    const mid = 1.0;
    const lowMidInterval = 1/8;   // Half the actual interval so the LEDs light in the middle of the interval
    const midHighInterval = 4/8;  // Half the actual interval so the LEDs light in the middle of the interval
    if (value>0.0) { LEDs++; }
...
StantonSCS3d.circleFlash = function(deck) {
    ...
    if (!StantonSCS3d.state.circleInvert) { StantonSCS3d.state.circleInvert=true; } else { StantonSCS3d.state.circleInvert=false; }
...
StantonSCS3d.circleLEDs = function(value) {
    ...
    const deck = StantonSCS3d.deck;

    const currentMode = StantonSCS3d.mode_store["[Channel"+deck+"]"];
    ...
    if (StantonSCS3d.trackDuration[deck]>30) {
        const trackTimeRemaining = ((1-value) * StantonSCS3d.trackDuration[deck]) | 0;
    ...
    let revtime = StantonSCS3d.revtime;

    if (StantonSCS3d.spinningLights==2) { revtime = revtime/2; }    // Use this for two lights
    const currentTrackPos = value * StantonSCS3d.trackDuration[deck];

    const revolutions = currentTrackPos/revtime;

    let light = ((revolutions-(revolutions|0))*16)|0;   // OR with 0 replaces Math.floor and is faster
