...
NovationDicer.effectThenContinue = function(channel, control, value, status, group) {
    if (value)  //Button pressed
    {
        ...
        if (group == "[Channel1]") { NovationDicer.effectsCH1++; } else { NovationDicer.effectsCH2++; }
    ...
    } else      //Button released
    {
        ...
        if (((NovationDicer.effectsCH1 == 1) && (group == "[Channel1]")) || ((NovationDicer.effectsCH2 == 1) && (group == "[Channel2]")))  //End the effect session and resume playing if it's the last effect running
        {
            if (group == "[Channel1]") {
                ...
                const track1Ms = engine.getValue(group, "duration") * 1000;   // Convert seconds to MS
                var newPlayPos = (NovationDicer.startPlayPosCH1 + (tDelta/track1Ms));
            } else {
                var tDelta = new Date() - NovationDicer.startTimeCH2;
                tDelta = tDelta + 28 + (tDelta * 0.00065);   //Compensate for wandering/delay
                tDelta = tDelta + (tDelta * (engine.getValue(group, "rate") * 0.1) * engine.getValue(group, "rate_dir"));   //Compensate for pitch
                const track2Ms = engine.getValue(group, "duration") * 1000;
...
NovationDicer.stutter = function(group) {
    if (engine.getValue(group, "pregain") != 0) { engine.setValue(group, "pregain", 0); } else {
        if (group == "[Channel1]") { engine.setValue(group, "pregain", NovationDicer.gainCH1); } else { engine.setValue(group, "pregain", NovationDicer.gainCH2); }
    }
};
...
NovationDicer.cueButton = function(channel, control, value, status, group) {
    if (value) {
        if (group == "[Channel1]") { midi.sendShortMsg(0x9c, 0x3d, NovationDicer.orange); } else { midi.sendShortMsg(0x9f, 0x3d, NovationDicer.orange); }

        engine.setValue(group, "cue_default", 1);
    } else {
        if (group == "[Channel1]") { midi.sendShortMsg(0x9c, 0x3d, NovationDicer.softOrange); } else { midi.sendShortMsg(0x9f, 0x3d, NovationDicer.softOrange); }
