        ...
        if (start != -1) {
            if (end != -1) {
                ...
                if (NumarkTotalControl.extendedLooping) {
                    const deck = NumarkTotalControl.groupToDeck(group);
...
NumarkTotalControl.loopExtendedAdjustment = function(group) {
    ...
    let start = engine.getValue(group, "loop_start_position");
    ...
    let len = currentPosition - start;

    // Calculate nearest beat
    const beatSamples = NumarkTotalControl.samplesPerBeat(group);
    let lenInBeats = len / beatSamples;
    if (lenInBeats > 1) {
...
NumarkTotalControl.quantizeLED = function(value, group, key) {
    const deck1 = engine.getValue("[Channel1]", "quantize");
    const deck2 = engine.getValue("[Channel2]", "quantize");
    if (deck1 == deck2) {
...
NumarkTotalControl.toggleQuantize = function(channel, control, value, status, group) {
    // Toggle setting
    if (value) {
        const newValue = !(engine.getValue("[Channel1]", "quantize") && engine.getValue("[Channel2]", "quantize"));
