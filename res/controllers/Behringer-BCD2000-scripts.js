...
BehringerBCD2000.init = function(id) { // called when the device is opened & set up
    ...
    if (BehringerBCD2000.UseAcceleration) { midi.sendShortMsg(0xB0, 0x63, 0x7F); } else { midi.sendShortMsg(0xB0, 0x63, 0x0); }
};
...
BehringerBCD2000.shutdown = function() {
    ...
    if (!BehringerBCD2000.UseAcceleration) { midi.sendShortMsg(0xB0, 0x63, 0x7F); }
};
...
BehringerBCD2000.jogWheel = function(channel, control, value, status, group) {
    ...
    if (BehringerBCD2000.escratch[deck]) {
        ...
        if (BehringerBCD2000.debug) { print(group + " scratch tick : " + scratchValue); }
        ...
        if (BehringerBCD2000.debug) { print(group + " pitching jog adjust : " + jogValue); }
...
BehringerBCD2000.scratchButton = function(channel, control, value, status, group) {
    ...
    if (BehringerBCD2000.debug) { print(group + " scratch enabled :" + BehringerBCD2000.escratch[deck]); }
...
BehringerBCD2000.loop = function(channel, control, value, status, group) {
    ...
    if (BehringerBCD2000.debug) { print(group + " " + action); }
