...
DigitalJockey2Controller.playTrack = function(channel, control, value) {
    ...
    if (value == DigitalJockey2Controller.keyPressed) {
        ...
        const currentlyPlaying = engine.getValue("[Channel"+channel+"]", "play");
...
DigitalJockey2Controller.EnableHeadPhone = function(channel, control, value) {
    const isHeadPhoneActive = engine.getValue("[Channel"+channel+"]", "pfl");
...
DigitalJockey2Controller.BassKillChannel1 = function(channel, control, value) {
    const deck = 1;
    if (value == DigitalJockey2Controller.keyPressed) {
        const isKillButtonIsActive = engine.getValue("[Channel"+deck+"]", "filterLowKill");
...
DigitalJockey2Controller.MidKillChannel1 = function(channel, control, value) {
    const deck = 1;
    if (value == DigitalJockey2Controller.keyPressed) {
        const isKillButtonIsActive = engine.getValue("[Channel"+deck+"]", "filterMidKill");
        if (isKillButtonIsActive == true) {
...
DigitalJockey2Controller.HighKillChannel1 = function(channel, control, value) {
    const deck = 1;
    if (value == DigitalJockey2Controller.keyPressed) {
        const isKillButtonIsActive = engine.getValue("[Channel"+deck+"]", "filterHighKill");
        if (isKillButtonIsActive == true) {
...
DigitalJockey2Controller.JogWheelHelper = function(channel, control, value) {
    ...
    if (control == 29) {
        if (DigitalJockey2Controller.scratchModeChannel1 == true && DigitalJockey2Controller.searchModeChannel1 == true) {
            ...
            if (currentlyPlaying) {
                if (value == 0) {
                    if (engine.getValue("[Channel1]", "rate_temp_up_small")) { engine.setValue("[Channel1]", "rate_temp_up_small", 0); }
                    if (engine.getValue("[Channel1]", "rate_temp_down_small")) { engine.setValue("[Channel1]", "rate_temp_down_small", 0); }
                }
            } else {
                if (value == 127) { engine.scratchEnable(1, 128, 33+1/3, 1.0/8, (1.0/8)/32); } else if (value == 0) { engine.scratchDisable(1); }
            }
            ...
            if (value == 0) {
                if (engine.getValue("[Channel1]", "fwd")) { engine.setValue("[Channel1]", "fwd", 0); }
                if (engine.getValue("[Channel1]", "back")) { engine.setValue("[Channel1]", "back", 0); }
            }
        }
    } else if (control == 89) {
        if (DigitalJockey2Controller.scratchModeChannel2 == true && DigitalJockey2Controller.searchModeChannel2 == true) {
            ...
            if (currentlyPlaying) {
                if (value == 0) {
                    if (engine.getValue("[Channel2]", "rate_temp_up_small")) { engine.setValue("[Channel2]", "rate_temp_up_small", 0); }
                    if (engine.getValue("[Channel2]", "rate_temp_down_small")) { engine.setValue("[Channel2]", "rate_temp_down_small", 0); }
                }
            } else {
                if (value == 127) { engine.scratchEnable(2, 128, 33+1/3, 1.0/8, (1.0/8)/32); } else if (value == 0) { engine.scratchDisable(2); }
            }
            ...
            if (value == 0) {
                if (engine.getValue("[Channel2]", "fwd")) { engine.setValue("[Channel2]", "fwd", 0); }
                if (engine.getValue("[Channel2]", "back")) { engine.setValue("[Channel2]", "back", 0); }
            }
        }
    }
};
...
DigitalJockey2Controller.JogWheel = function(channel, control, value) {
	 ...
	 */
    const jogValue = (value - 64); //DigitalJockey2Controller.WheelSensitivity;
    ...
    if (channel == 1) {
	    if (DigitalJockey2Controller.scratchModeChannel1 == true && DigitalJockey2Controller.searchModeChannel1 == true) {
		    ...
		    var rtd = engine.getValue("[Channel1]", "rate_temp_down_small");
	            if (jogValue > 0) {
	                if (rtd) { engine.setValue("[Channel1]", "rate_temp_down_small", 0); }
                    if (rtu != 1) { engine.setValue("[Channel1]", "rate_temp_up_small", 1); }

	            } else if (jogValue < 0) {
	                if (rtu) { engine.setValue("[Channel1]", "rate_temp_up_small", 0); }
	                if (rtd != 1) { engine.setValue("[Channel1]", "rate_temp_down_small", 1); }
	            } else { return; }
	        } else {
            ...
            var currentlyPlaying = engine.getValue("[Channel1]", "play");
	        if (currentlyPlaying) {
		    if (jogValue == 0) { return; }
		    var fwd = engine.getValue("[Channel1]", "fwd");
		    var back = engine.getValue("[Channel1]", "back");
		    if (jogValue > 0) {
                    if (back) { engine.setValue("[Channel1]", "back", 0); }
                    if (fwd != 1) { engine.setValue("[Channel1]", "fwd", 1); }
		    } else if (jogValue < 0) {
                    if (fwd) { engine.setValue("[Channel1]", "fwd", 0); }
                    if (back != 1) { engine.setValue("[Channel1]", "back", 1); }
		    }
            } else {
		    var playpos = engine.getValue("[Channel1]", "playposition");
		    if (jogValue > 0) {
		        if (playpos < 1 - DigitalJockey2Controller.SearchSensitivity) { playpos += DigitalJockey2Controller.SearchSensitivity; } else { playpos = 1; }
		    } else if (jogValue < 0) {
                    if (playpos > DigitalJockey2Controller.SearchSensitivity) { playpos -= DigitalJockey2Controller.SearchSensitivity; } else { playpos = 0; }
		    }
    ...
    if (channel == 2) {
	    if (DigitalJockey2Controller.scratchModeChannel2 == true && DigitalJockey2Controller.searchModeChannel2 == true) {
		    ...
		    var rtd = engine.getValue("[Channel2]", "rate_temp_down_small");
	            if (jogValue > 0) {
	                if (rtd) { engine.setValue("[Channel2]", "rate_temp_down_small", 0); }
                    if (rtu != 1) { engine.setValue("[Channel2]", "rate_temp_up_small", 1); }

	            } else if (jogValue < 0) {
	                if (rtu) { engine.setValue("[Channel2]", "rate_temp_up_small", 0); }
	                if (rtd != 1) { engine.setValue("[Channel2]", "rate_temp_down_small", 1); }
	            } else {}
	        } else {
		    ...
		    if (jogValue > 0) {
                    if (back) { engine.setValue("[Channel2]", "back", 0); }
                    if (fwd != 1) { engine.setValue("[Channel2]", "fwd", 1); }
		    } else if (jogValue < 0) {
                    if (fwd) { engine.setValue("[Channel2]", "fwd", 0); }
                    if (back != 1) { engine.setValue("[Channel2]", "back", 1); }
		    }
            } else {
...
DigitalJockey2Controller.isChannel2_Playing = function(value) {
    if (value == 0) {
        ...
        if (DigitalJockey2Controller.CUP_Button2_IsActive == false) { midi.sendShortMsg(0x90, 0x55, DigitalJockey2Controller.ledOn); }   // Turn on the Play LED2 on
    }
};
DigitalJockey2Controller.isChannel1_Cue_Active = function(value) {
    if (value == 0) {
        if (DigitalJockey2Controller.CUP_Button1_IsActive == true) { midi.sendShortMsg(0x90, 0x17, DigitalJockey2Controller.ledOn); } //Turn CUP LED1 on
        midi.sendShortMsg(0x90, 0x18, DigitalJockey2Controller.ledOff); //Turn CUE LED1 off
    } else {
        ...
        } else { midi.sendShortMsg(0x90, 0x18, DigitalJockey2Controller.ledOn); } //Turn CUE LED1 on
...
DigitalJockey2Controller.isChannel2_Cue_Active = function(value) {
    ...
    if (value == 0) {
        if (DigitalJockey2Controller.CUP_Button2_IsActive == true) { midi.sendShortMsg(0x90, 0x53, DigitalJockey2Controller.ledOn); }  //Turn CUP LED2 on
        midi.sendShortMsg(0x90, 0x54, DigitalJockey2Controller.ledOff); //Turn CUE LED2 off
    } else {
        //if CUP LED is active leave, we can switch off CUE Button
        ...
        } else { midi.sendShortMsg(0x90, 0x54, DigitalJockey2Controller.ledOn); } //Turn CUE LED2 on

    }
};
DigitalJockey2Controller.SelectNextTrack_or_prevTrack = function(channel, control, value, status) {
    if (value == 65) { engine.setValue("[Playlist]", "SelectNextTrack", 1); } else { engine.setValue("[Playlist]", "SelectPrevTrack", 1); }
...
DigitalJockey2Controller.OnFilterHigh_KillButton2 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x50, DigitalJockey2Controller.ledOn); } //HighFilterKill
    else { midi.sendShortMsg(0x90, 0x50, DigitalJockey2Controller.ledOff); } //HighFilterKill
};
DigitalJockey2Controller.OnFilterMid_KillButton2 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x51, DigitalJockey2Controller.ledOn); } //MidFilterKill
    else { midi.sendShortMsg(0x90, 0x51, DigitalJockey2Controller.ledOff); } //MidFilterKill
};
DigitalJockey2Controller.OnFilterLow_KillButton2 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x52, DigitalJockey2Controller.ledOn); } //LowFilterKill
    else { midi.sendShortMsg(0x90, 0x52, DigitalJockey2Controller.ledOff); } //LowFilterKill
};

DigitalJockey2Controller.OnFilterHigh_KillButton1 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x14, DigitalJockey2Controller.ledOn); } //HighFilterKill
    else { midi.sendShortMsg(0x90, 0x14, DigitalJockey2Controller.ledOff); } //HighFilterKill
};
DigitalJockey2Controller.OnFilterMid_KillButton1 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x15, DigitalJockey2Controller.ledOn); } //HighFilterKill
    else { midi.sendShortMsg(0x90, 0x15, DigitalJockey2Controller.ledOff); } //HighFilterKill
};
DigitalJockey2Controller.OnFilterLow_KillButton1 = function(value) {
    if (value == 1) { midi.sendShortMsg(0x90, 0x16, DigitalJockey2Controller.ledOn); } //HighFilterKill
    else { midi.sendShortMsg(0x90, 0x16, DigitalJockey2Controller.ledOff); } //HighFilterKill
};
...
DigitalJockey2Controller.ReloopExit = function(channel, control, value, status, group) {
    ...
    const actBeatLoop = engine.getValue(group, "beatloop");
...
DigitalJockey2Controller.Flanger1 = function(channel, control, value, status, group) {
    if (value == DigitalJockey2Controller.keyPressed) {
        const flanger = engine.getValue("[Channel1]", "flanger");
	    if (flanger) {
...
DigitalJockey2Controller.Flanger2 = function(channel, control, value, status, group) {
    if (value == DigitalJockey2Controller.keyPressed) {
        const flanger = engine.getValue("[Channel2]", "flanger");
	    if (flanger) {
...
DigitalJockey2Controller.LoadSelectedTrack = function(channel, control, value) {
    /* added by Jones */
    if (control == 19) {
        if (engine.getValue("[Channel1]", "play") == 0) { engine.setValue("[Channel1]", "LoadSelectedTrack", 1); } else { print("no load to deck1!\n"); }
    } else if (control == 79) {
        if (engine.getValue("[Channel2]", "play") == 0) { engine.setValue("[Channel2]", "LoadSelectedTrack", 1); } else { print("no load to deck2!\n"); }
    }
};

DigitalJockey2Controller.PitchControl = function(channel, control, value) {
    const pitch_value = -1.0 + 2.0 * ((value * 8 + control / 16) / 1024.0);
...
DigitalJockey2Controller.KeyLock = function(channel, control, value) {
    const isKeyLock = engine.getValue("[Channel"+channel+"]", "keylock");
    if (value == DigitalJockey2Controller.keyPressed) {
        if (isKeyLock == 1) {
...
DigitalJockey2Controller.getPrevBeatLoop = function(beatloop) {
    ...
    for (i = 0; i < DigitalJockey2Controller.beatloops.length; i++) {
        if (DigitalJockey2Controller.beatloops[i] == beatloop) {
            if (i > 0) { return DigitalJockey2Controller.beatloops[i - 1]; } else { return DigitalJockey2Controller.beatloops[0]; }
        }
    }
    return "0";
};
...
DigitalJockey2Controller.getNextBeatLoop = function(beatloop) {
    ...
    for (i = 0; i < DigitalJockey2Controller.beatloops.length; i++) {
        if (DigitalJockey2Controller.beatloops[i] == beatloop) {
            if (i < DigitalJockey2Controller.beatloops.length - 1) { return DigitalJockey2Controller.beatloops[i + 1]; } else { return DigitalJockey2Controller.beatloops[DigitalJockey2Controller.beatloops.length - 1]; }
        }
    }
    return "0";
};
...
DigitalJockey2Controller.LoopPlusMinus = function(channel, control, value, status, group) {
    ...
    if (actBeatLoop != "0") {
        if (value == 0x3F) {
            const prevBeatLoop = DigitalJockey2Controller.getPrevBeatLoop(actBeatLoop);
            print("LoopPlusMinus.prevBeatLoop=" + prevBeatLoop);
            engine.setValue(group, "beatloop", prevBeatLoop);
        } else if (value == 0x41) {
            const nextBeatLoop = DigitalJockey2Controller.getNextBeatLoop(actBeatLoop);
