...
RMX.init = function() {
    ...
    const print_unhandled_button = function(g, e, v) { print("Unhandled button"); };
    ...
    const filterKill = function(g, e, v) {
        engine.setValue(g, e, !engine.getValue(g, e));
    };
    ...
    const pipe = function(g, e, v) { c.send(g, e, v); };
...
RMX.scroll_tracks = function(g, e, v) {
    if (v > 0) {
        ...
        const direction = e == "menu_down" ? "SelectNextTrack" : "SelectPrevTrack";
        engine.setValue("[Playlist]", direction, 1);

        if (!RMX.scroll_timer) {
            const callback = "RMX.scroll_tracks(\"[Playlist]\",\"" + e + "\"," + v + ")";
...
RMX.jog = function(g, e, v, ctrl) {
    ...
    if (RMX.scratch_enabled) {
        ...
        const deck = parseInt(g.substring(8, 9));
        ...
        const callback = "RMX.stopScratching(\"" + g + "\")";
...
RMX.send = function(g, e, v) {
    if ((ctrl = this.leds[g + e]) !== undefined) {
        ...
        const tmp = this.cache_out[ctrl.packetid];
...
RMX.feedbackData = function(v, g, e) {
    if (RMX.feedbacks[g + e] !== undefined) {
        for (const func in RMX.feedbacks[g + e]) {
...
RMX.incomingData = function(data, length) {
    ...
    for (let i=1; i<length; i++) {
        ...
        if ((p === undefined || data[i] != p[i]) && c.controls[i] !== undefined) {
            ...
            for (const key in c.controls[i]) {
                const control = c.controls[i][key];
            ...
            control.changed(data[i])) {
                    ...
                    // check for callbacks
                    const callbacks = c.callbacks[control.name];

                    if (callbacks !== undefined) {
                        for (let j=0; j<callbacks.length; j++) {

                            const cb = callbacks[j][1];

                            if (typeof(cb) === "function") {

                                // check we need to call for this value change:
                                // all, press, release
                                const v       = callbacks[j][0];
                                const all     = v == "all";
                                const press   = v == "press"   && control.value  >  0;
                                const release = v == "release" && control.value === 0;
