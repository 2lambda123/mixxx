...
WirelessDJ.meter = function(value, group, key) {
    const deck = WirelessDJ.groupToDeck(group);
    // there is 11 leds in WirelessDJ volume meter.

    const val = Math.round(parseFloat(value) * 11) * 128.0/11.0;
...
WirelessDJ.jog_on = function(channel, control, value, status, group) {
    ...
    if (status == 0x90) {
        ...
        WirelessDJ.magicStripeMode[deck] &= ~0x2;
        const position = engine.getValue(group, "wheel");
        if (position != 0) {
...
WirelessDJ.magic_stripe_lsb = function(channel, control, value, status, group) {
    ...
    if (WirelessDJ.magicCurMSB[deck] == undefined) { return; }

    const firstValue = (WirelessDJ.prevMagicValue[deck] == undefined);

    const adjustedValue = (WirelessDJ.magicCurMSB[deck] * 128 + value) / 16384.0;
    let diff = WirelessDJ.prevMagicValue[deck] - adjustedValue;
    if (diff > 0.9) { diff -= 1; }
    if (diff < -0.9) { diff += 1; }
