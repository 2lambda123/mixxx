...
XoneK2.init = function(id) {
    let channel = XoneK2.decksInMiddleMidiChannel;
...
XoneK2.shutdown = function(id) {
    const turnOff = function(component) {
        component.send(0);
    };
    for (let z = 1; z <= 4; z++) {
...
XoneK2.decksBottomLeftEncoder = function(channel, control, value, status) {
    if (!XoneK2.controllers[channel].isShifted) {
        if (!XoneK2.controllers[channel].leftEncoderIsPressed) {
            let bpm = engine.getValue("[InternalClock]", "bpm");
            if (value === 1) {
                bpm += 0.1;
            } else {
                bpm -= 0.1;
            ...
            let mix = engine.getValue("[Master]", "headMix");
            if (value === 1) {
                mix += 1;
            } else {
                mix -= 1;
        ...
        let gain = engine.getValue("[Master]", "headGain");
        if (value === 1) {
            gain += 0.025;
        } else {
            gain -= 0.025;
...
XoneK2.decksBottomRightEncoder = function(channel, control, value, status) {
    if (!XoneK2.controllers[channel].isShifted) {
        const bpm = engine.getValue("[InternalClock]", "bpm");
        if (value === 1) {
        ...
        let gain = engine.getValue("[Master]", "gain");
        if (value === 1) {
            gain += 0.025;
        } else {
            gain -= 0.025;
...
XoneK2.Deck = function(column, deckNumber, midiChannel) {
    ...
    const CueAndSeekButton = function(options) {
    ...
    const setGroup = function(component) {
        if (component.group === undefined) {
            component.group = theDeck.deckString;
        }
    };

    for (const memberName in this.bottomButtonLayers) {
...
XoneK2.decksLayerButton = function(channel, control, value, status) {
    if (!XoneK2.controllers[channel].isShifted) {
        ...
        if (components.Button.prototype.isPress(channel, control, value, status)) {
            ...
            const newLayer = XoneK2.deckBottomButtonLayers[XoneK2.controllers[channel].deckLayerIndex];
...
XoneK2.EffectUnit = function(column, unitNumber, midiChannel, twoDeck) {
    ...
    const libraryEffectUnit = new components.EffectUnit([unitNumber], false, {
        unfocused: XoneK2.color.red,
        focusChooseMode: XoneK2.color.green,
        focused: XoneK2.color.amber,
    });

    const unitString = "[EffectRack1_EffectUnit" + unitNumber + "]";
    ...
    this.unitFocusButton = new components.Button({
        input: function(channel, control, value, status) {
            if (this.isPress(channel, control, value, status)) {
                ...
                for (let x = 1; x <= 4; ++x) {
                    const effectUnitColumn = XoneK2.controllers[channel].columns[x];
    ...
    this.focusUnit = function(focusedUnitNumber) {
        ...
        for (let n = 0; n <= 0xF; n++) {
            const col = XoneK2.controllers[n].columns[column];
