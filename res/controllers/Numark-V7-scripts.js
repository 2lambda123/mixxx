...
NumarkV7.FxSliderA = function() {
    const currentvalue = (NumarkV7.FxSliderAFineParse+NumarkV7.FxSliderACoarseParse);
...
NumarkV7.FxSliderB = function() {
    const currentvalue = (NumarkV7.FxSliderBFineParse+NumarkV7.FxSliderBCoarseParse);
    engine.setValue("[Flanger]", "lfoDepth", currentvalue);
};
NumarkV7.FxParam = function(channel, control, value, status, group) {
    const currentvalue = engine.getValue("[Flanger]", "lfoPeriod");
...
NumarkV7.FxSelect = function(channel, control, value, status, group) {
    const currentvalue = engine.getValue("[Flanger]", "lfoDelay");
...
NumarkV7.PitchA = function() {
    let currentvalue = (NumarkV7.PitchACoarseParse+NumarkV7.PitchAFineParse);
...
NumarkV7.PitchAMotor = function() {
    const rateAbs = (engine.getValue("[Channel1]", "rate"))*(engine.getValue("[Channel1]", "rateRange"));
    ...
    if (engine.getValue("[Channel1]", "rate") > 0) {
        const ratePos = Math.floor(rateAbs * 63);
        const ratePos1 = Math.floor(((rateAbs * 63) - ratePos) * 127);
 		midi.sendShortMsg(0xB0, 0x49, (0x00 + ratePos));
        midi.sendShortMsg(0xB0, 0x69, (0x00 + ratePos1));
        midi.sendShortMsg(0xB0, 0x37, 0x00);
    }
    if (engine.getValue("[Channel1]", "rate") < 0) {
        const rateNeg0 = (1 - (rateAbs * (-1)));
        const rateNeg = ((Math.floor(rateNeg0 * 63)) + 65);
        const rateNeg1 = Math.floor((((rateNeg0 * 63) + 65) - rateNeg) * 127);
...
NumarkV7.PitchB = function() {
    let currentvalue = (NumarkV7.PitchBCoarseParse+NumarkV7.PitchBFineParse);
    if ((currentvalue < 0.01)&&(currentvalue > -0.01)) {
        currentvalue = 0.00;
    }
...
NumarkV7.PitchBMotor = function() {
    const rateAbs = (engine.getValue("[Channel2]", "rate"))*(engine.getValue("[Channel2]", "rateRange"));
...
NumarkV7.RateRangeA = function(channel, control, value, status, group) {
    if (value == 0x7F) {
        const currentRange = Math.round(engine.getValue("[Channel1]", "rateRange")*100)/100;
...
NumarkV7.RateRangeB = function(channel, control, value, status, group) {
    if (value == 0x7F) {
        const currentRange = Math.round(engine.getValue("[Channel2]", "rateRange")*100)/100;
        print("Current range="+currentRange);
        switch (true) {
        case (currentRange<NumarkV7.RateRanges[0]):
...
NumarkV7.KeylockA = function() {
    const keylock = engine.getValue("[Channel1]", "keylock");
    if (keylock == 1) {
...
NumarkV7.KeylockB = function() {
    const keylock = engine.getValue("[Channel2]", "keylock");
    if (keylock == 1) {
...
NumarkV7.TempoLeanLED = function() {
    const BeatMatch = (engine.getValue("[Channel2]", "bpm")-engine.getValue("[Channel1]", "bpm"));
...
NumarkV7.LoopShiftUpA = function(channel, control, value, status, group) {
    if (value == 0x7F) {
        const loopStart = engine.getValue("[Channel1]", "loop_start_position");
        const loopEnd = engine.getValue("[Channel1]", "loop_end_position");
...
NumarkV7.LoopShiftUpB = function(channel, control, value, status, group) {
    if (value == 0x7F) {
        const loopStart = engine.getValue("[Channel2]", "loop_start_position");
        const loopEnd = engine.getValue("[Channel2]", "loop_end_position");
        const loopLength = (loopEnd - loopStart);
...
NumarkV7.ScratchA = function(channel, control, value, status, group) {
    const diff = value - NumarkV7.ScratchDiffA;
    let ticks = 0;
    switch (true) {
    case (diff == (-0x7F)):
        ticks = 1;
...
NumarkV7.ScratchB = function(channel, control, value, status, group) {
    const diff = value - NumarkV7.ScratchDiffB;
    let ticks = 0;
    switch (true) {
    case (diff == (-0x7F)):
        ticks = 1;
