...
HerculesMp3.ledhelper = function() {
    ...
    if (led_array.length > 0) {
        const nowled = led_array[0];
        const nowstatus = HerculesMp3.controls.outputs[nowled].status;
        led_array.shift();
    ...
    if (enable_beats) {
        const beatA = HerculesMp3.controls.outputs[0x0A].beatlen;
        const beatB = HerculesMp3.controls.outputs[0x04].beatlen;
...
HerculesMp3.hotcue = function(deck, control, value) {
    ...
    if (value) {
        ...
        if (!is_hold_hotcue[control]) {
            if (!engine.getValue("[Channel"+deck+"]", hotcue_string+"enabled")) { engine.setValue("[Channel"+deck+"]", hotcue_string+"activate", 1); } else { engine.setValue("[Channel"+deck+"]", hotcue_string+"goto", 1); }
        }
    }
};
...
HerculesMp3.mode = function(group, control, value, status) {
    ...
    const n = HerculesMp3.controls.inputs[control].channel;
...
HerculesMp3.fx = function(group, control, value, status) {
    ...
    const fxdeck = n==1 ? 0x07 : 0x01;

    const currentMode = HerculesMp3.controls.inputs[fxdeck].mode;
...
HerculesMp3.pitchpot = function(group, control, value, status) {
    ...
    if (value) {
        let increment = (max-min)/50;
        increment = (value <= 0x3F) ? increment : increment * -1;
        let newValue = engine.getValue(chan, action) + increment;
    ...
    if (scratch[n]) {
        ...
        if (value) {
            if (!engine.getValue(Deck[n], "play")) {
                let jogValue = value >=0x40 ? value - 0x80 : value;
...
HerculesMp3.joystick = function(group, control, value, status) {
    if (!value) { engine.setValue("[Playlist]", "SelectPrevTrack", 1); }
    if (value == 0x7F) { engine.setValue("[Playlist]", "SelectNextTrack", 1); }
};
...
HerculesMp3.loadSelectedTrack = function(group, control, value, status) {
    if (value) { // Only do stuff when pushed, not released.
        const deck = HerculesMp3.controls.inputs[control].channel;
...
HerculesMp3.bblinkA = function(csecA) {
    const actposA = engine.getValue(Deck[1], "duration")*engine.getValue(Deck[1], "playposition")*100;
...
HerculesMp3.bblinkB = function(csecB) {
    const actposB = engine.getValue(Deck[2], "duration")*engine.getValue(Deck[2], "playposition")*100;
