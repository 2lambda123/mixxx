...
function PioneerCDJController() {
    ...
    this.registerOutputPackets = function() {
        ...
        // anyway when implemented. When I know what data is sent, I
        // will add a helper function to actually do something with it!
        const waveform_packet_datalen = 400;
        packet = new HIDPacket("waveform", 0x1, undefined, [0x2]);
        for (var i=0; i<waveform_packet_datalen; i++) { packet.addControl("hid", "byte_"+i, i, "B"); }
        //this.controller.registerOutputPacket(packet);
    ...
    this.initializeHIDController = function() {
        ...
        const packet = controller.OutputPackets.request_hid_mode;
        if (packet!=undefined) {
            packet.send();
        }
...
PioneerCDJHID.init = function(id) {
    ...
    const controller = PioneerCDJHID.controller;
...
PioneerCDJHID.updateLED = function(value, group, key) {
    const controller = PioneerCDJHID.controller;
    if (value==1) { controller.setOutput("deck", key, controller.LEDColors.on, true); } else { controller.setOutput("deck", key, controller.LEDColors.off, true); }
};
...
PioneerCDJHID.connectDeck = function() {
    const controller = PioneerCDJHID.controller;
    const group = controller.resolveDeckGroup(controller.activeDeck);
    const output_packet = controller.getOutputPacket("lights");
...
PioneerCDJHID.timeMode = function(field) {
    const controller = PioneerCDJHID.controller;
    if (field.value==controller.buttonStates.released) { return; }
    PioneerCDJHID.remain_mode = (PioneerCDJHID.remain_mode!=true);
    if (PioneerCDJHID.remain_mode) {
        controller.setOutput("hid", "remain", 1);
    } else {
        controller.setOutput("hid", "remain", 0);
...
PioneerCDJHID.setDuration = function(value) {
    ...
    } else { PioneerCDJHID.track_length = value; }
};
...
PioneerCDJHID.setRate = function(value) {
    const controller = PioneerCDJHID.controller;
    const group = controller.resolveDeckGroup(controller.activeDeck);
    const range = Math.floor(100*engine.getValue(group, "rateRange"));
...
PioneerCDJHID.durationCallback = function(value, group, key) {
    const output_packet = PioneerCDJHID.controller.getOutputPacket("lights");
...
PioneerCDJHID.track = function(field) {
    ...
    if (field.name=="previous_track") {
        const position = PioneerCDJHID.track_length -
