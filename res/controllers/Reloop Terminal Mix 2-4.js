...
TerminalMix.samplerVolume = function(channel, control, value) {
    ...
    for (let i=engine.getValue("[Master]", "num_samplers"); i>=1; i--) {
...
TerminalMix.pitchRange = function(channel, control, value, status, group) {
    ...
    const currentRange = Math.round(engine.getValue(group, "rateRange")*100)/100;
    const currentPitch = engine.getValue(group, "rate") * currentRange;
    const items = TerminalMix.pitchRanges.length;
    for (i=0; i<items; i++) {
...
TerminalMix.samplerPlayFlash = function() {
    ...
    for (let i=1; i<=4; i++) { // 4 samplers
        ...
        for (let j=1; j<=4; j++) {  // Same buttons on all 4 controller decks
...
TerminalMix.crossFader = function(channel, control, value, status, group) {
    const cfValue = script.absoluteNonLin(value, -1, 0, 1);
...
TerminalMix.tapLED = function(deck, value) {
    deck--;
    if (value>0) { midi.sendShortMsg(0x90+deck, 0x0A, 0x7F); } else { midi.sendShortMsg(0x90+deck, 0x0A, 0); }
};
