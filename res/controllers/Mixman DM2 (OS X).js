...
DM2.wheel = function(channel, control, value, status) {
    // 14bit values
    let newValue = (value << 7) | control;
    if (newValue >= 8192) { newValue = newValue - 16384; }
...
DM2.playlist = function(channel, control, value, status) {
    // 7bit values
    let newValue=value;
    if (value >= 64) { newValue = newValue - 128; }
...
DM2.filter = function(channel, control, value, status) {
    const deck = DM2.cc2deck[control];
    // 7bit values
    if (value >= 64) { value = value - 128; }

    let f = engine.getValue("[Channel" + deck + "]", DM2.cc2mixxx[control]);
    f += value / 256;
    if (f > 4.0) { f = 4.0; } else if (f < 0.0) { f = 0.0; }
...
DM2.filterKill = function(channel, control, value, status) {
    print("control "+control);
    if (DM2.filter_active) { DM2.filter_active = 0; } else {
...
DM2.trigger = function(channel, control, value, status) {
    ...
    if (DM2.filter_active) { DM2.filter_active = 0; } else {
        const f = engine.getValue("[Channel" + DM2.button2deck[control] + "]", DM2.button2mixxx[control]);
        engine.setValue("[Channel" + DM2.button2deck[control] + "]",
            DM2.button2mixxx[control], 1);
        let i;
        for (i = 0; i < 10000; i++) {}
