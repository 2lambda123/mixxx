...
LPD8RK.setLeds = function() {
    ...
    for (var id in LPD8RK.hotcues) {
        for (const bank in LPD8RK.hotcues[id]) {
...
LPD8RK.hotcueButton = function(channel, control, value, status, group) {
    ...
    const thecue = LPD8RK.hotcues[status.toString(16).toLowerCase()+control.toString(16).toLowerCase()][LPD8RK.hotcueBank];
...
LPD8RK.loopButton = function(channel, control, value, status, group) {
    ...
    //activate beatloop
    const theloop = LPD8RK.loops[status.toString(16).toLowerCase()+control.toString(16).toLowerCase()];
...
LPD8RK.hotcueBankDial = function(channel, control, value, status, group) {
    ...
    if (LPD8RK.oldHotcueBank != LPD8RK.hotcueBank) { //check if the bank's changed.  If it has, change the LEDs - LPD8RK.oldHotcueBank != LPD8RK.hotcueBank
        for (const id in LPD8RK.hotcues) {
            for (const bank in LPD8RK.hotcues[id][1]) {
                var status = LPD8RK.hotcues[id][1][2];
                const ctrl = LPD8RK.hotcues[id][1][3];
...
LPD8RK.loopminus = function(channel, control, value, status, group) {
    ...
    if (LPD8RK.loopbuttonDown !== true) { engine.setValue(group, "loop_halve", 1); engine.setValue(group, "loop_halve", 0); return false; }//shrink loop if no loop button down
        ...
        //move loop
        const interval =    LPD8RK.loopmove*engine.getValue(group, "track_samples")/engine.getValue(group, "duration");
...
LPD8RK.beatjump = function(channel, control, value, status, group) {
    ...
    const numbeats = LPD8RK.beatjumpstep;
    const backseconds = numbeats*(1/(engine.getValue(group, "bpm")/60));
    const backsamples = backseconds*engine.getValue(group, "track_samples")/engine.getValue(group, "duration");
    const newpos = curpos-(backsamples+engine.getValue("Master", "latency"));
