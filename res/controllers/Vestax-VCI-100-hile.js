...
HileVci100.debug = function(message) {
    if (!HileVci100.debugging) { return; }
...
HileVci100.init = function(id) {
    ...
    for (const control in HileVci100.NOTE_ON_CONTROLS) {
        const name = HileVci100.NOTE_ON_CONTROLS[control];
        if (name in HileVci100.buttonMap) {
            var button = HileVci100.buttonMap[name];
            if (IGNORE_LED_UPDATES.indexOf(button.control)!=-1) { continue; }
    ...
    for (i=1; i<=7; i++) {
        const group = HileVci100.activeLooperGroup;
        const button_name = "loop_button_" + i;
...
HileVci100.shutdown = function(id) {
    for (const button_name in HileVci100.buttonMap) {
        const button = HileVci100.buttonMap[name];
        if (IGNORE_LED_UPDATES.indexOf(button.control)!=-1) { continue; }
        engine.connectControl(
...
HileVci100.linkButton = function(name, group, control, toggle, callback) {
    ...
    for (const n in HileVci100.NOTE_ON_CONTROLS) {
...
HileVci100.LEDCallback = function(value, group, control) {
    for (const button_name in HileVci100.buttonMap) {
        const button = HileVci100.buttonMap[button_name];
        if (button.group!=group || button.control!=control) { continue; }
        const status = (value) ? 0x7f : 0x0;
...
HileVci100.faders = function(channel, control, value, status, group) {
    ...
    const fader = HileVci100.faderMap[name];
    if (fader==undefined) {
    ...
    if (fader.callback) { fader.callback(fader, value); } else { engine.setValue(fader.group, fader.control, value); }
};
...
HileVci100.buttons = function(channel, control, value, status, group) {
    ...
    const activate = /deck[0-9]+_activate/;
    ...
    if (button.toggle) {
        if (value==HileVci100.buttonStates.released) { return; }
        ...
        value = (value==HileVci100.buttonStates.pressed);
    }
...
HileVci100.cuestop = function(button, value) {
    const modifiers = HileVci100.modifiers[button.group];
    if (value) {
...
HileVci100.updateLooperLEDs = function() {
    ...
    for (i=1; i<=4; i++) {
        const name = "beatloop_" + HileVci100.beatloopSizes[i];
        const buttonname = "loop_button_" + (i+3);
...
HileVci100.active = function(button, value) {
    if (!value) { return; }
    if (button.group==HileVci100.activeLooperGroup) { return; }
    const modifiers = HileVci100.modifiers[button.group];
    HileVci100.activeLooperGroup = button.group;
    const group = HileVci100.activeLooperGroup;
    const inactive_group = (group=="[Channel1]") ? "[Channel2]" : "[Channel1]";
...
HileVci100.looper = function(button, value) {
    ...
    if (button.control=="loop_out") {
        ...
        const status = (button.group==group) ? 0x7f : 0x0;
        midi.sendShortMsg(0x90, button.midino, status);
...
HileVci100.beatloop = function(button, value) {
    ...
    const modifiers = HileVci100.modifiers[button.group];
    const group = HileVci100.activeLooperGroup;
    const control = button.control.replace("_enabled", "_activate");
