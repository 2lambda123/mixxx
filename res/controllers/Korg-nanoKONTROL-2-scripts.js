...
NK2.button = function button(channel, control, value, status, group) {
    ...
    const theControl=NK2.getControl(control);
...
NK2.knob = function knob(channel, control, value, status, group) {
    ...
    const command = theControl.pressEval;
    if (command!=false) { eval(command); }
};
...
NK2.muteLEDs = function muteLEDs(value, group, control) {
    ...
    for (const key in NK2.Controls) {
        if (NK2.Controls[key].LEDhookControl==control && NK2.Controls[key].LEDhookGroup==group) {
            const midino=key;
            const state=(value>0 || NK2.deckData[group].muteVol==-1)?0:1;
            midi.sendShortMsg(NK2.midiChannel, midino, state*127);
            NK2.Controls[key].LEDstate=state;
...
NK2.getFunctionName = function getFunctionName() { //return name of calling function
    const re = /function (.*?)\(/;
    const s = NK2.getFunctionName.caller.toString();
    const m = re.exec(s);
    return m[1];
};
...
NK2.getControl = function getControl(control) {
    ...
    let theControl=NK2.Controls[control][NK2.curNSMR][NK2.getCurModCode()];//get control for current mod settings
...
NK2.getCurModCode = function getCurModCode(control) { //returns the current mod code value
    ...
    const codestr=NK2.cycleState.toString()+NK2.bankSelectState.toString()+NK2.MOD1state.toString()+NK2.MOD2state.toString();
...
NK2.knobAdjust = function knobAdjust(group, control, value, min, max) {
    ...
    const range=max-min;
    let newValue=min+((value/127)*range);
...
NK2.logKnobAdjust = function logKnobAdjust(group, control, value, min, max) {
    ...
    const range=max-min;
    const adjustedValue=value/127;//adjust range so it's on a scale of 0 to 1 - logarithmic
    let newValue=min+(range*Math.pow((adjustedValue), 2));
...
NK2.toggleBinaryControlAll = function toggleBinaryControlAll(control) {
    ...
    const newstate = (engine.getValue(NK2.Deck[1], control)==0)?1:0;
    for (let i=1; i<9; i++) {
...
NK2.wavezoomAll = function wavezoomAll(value) {
    ...
    const range=6-1;
    let newValue=Math.round(1+((value/127)*range));
...
NK2.binControlPress = function binControlPress(deck, control) { //press button - toggle binary control for a deck
    ...
    const curstate=engine.getValue(deck, control);
...
NK2.cueLoop = function cueLoop(cue) { //jump to hotcue and start loop
    ...
    if (engine.getValue(NK2.Deck[NK2.curDeck], "hotcue_"+cue+"_enabled")!==1) { //set hotcue and start loop
        ...
        const startpos=engine.getValue(NK2.Deck[NK2.curDeck], "hotcue_"+cue+"_position");
        const loopseconds = NK2.cueLoopLen*(1/(engine.getValue(NK2.Deck[NK2.curDeck], "bpm")/60));
        const loopsamples = loopseconds*engine.getValue(NK2.Deck[NK2.curDeck], "track_samplerate")*2;//*2 to compensate for stereo samples
...
NK2.beatjump = function(jumplen) {
    ...
    const curpos = engine.getValue(NK2.Deck[NK2.curDeck], "playposition")*engine.getValue(NK2.Deck[NK2.curDeck], "track_samples");
    const numbeats = jumplen;
    const backseconds = numbeats*(1/(engine.getValue(NK2.Deck[NK2.curDeck], "bpm")/60));
    const backsamples = backseconds*engine.getValue(NK2.Deck[NK2.curDeck], "track_samplerate")*2;//*2 to compensate for stereo samples
...
NK2.loopDial = function loopDial(value, deck) {
    ...
    const newLoopLen=NK2.beatloopLengths[value];
    if (engine.getValue(deck, "loop_enabled")==true) {
        const startpos=engine.getValue(deck, "loop_start_position");
        const loopseconds = newLoopLen*(1/(engine.getValue(deck, "bpm")/60));
        const loopsamples = loopseconds*engine.getValue(deck, "track_samplerate")*2;//*2 to compensate for stereo samples
        const endpos=startpos+loopsamples;
        engine.setValue(NK2.Deck[NK2.curDeck], "loop_end_position", endpos);
    } else {
...
NK2.initControls = function initControls(obj) { //initialize controls array
    ...
    for (const x in obj) {
        ...
        for (const y in NK2.Banks) { //iterate through bank modes
            NK2.Controls[obj[x]][NK2.Banks[y]]={};//init empty

            for (let z=0; z<16; z++) { //iterate through mod codes
 ...
 */
    ...
    const shortlooplens={1: 0.03125, 2: 0.0625, 3: 0.125, 4: 0.25, 5: 0.5, 6: 1, 7: 2, 8: 4};//array of shorter loop and beatjump lengths
    for (var i=1; i<9; i++) {
