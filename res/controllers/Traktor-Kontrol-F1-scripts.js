...
function KontrolF1Controller() {
    ...
    this.setButtonBrightness = function(name, value) {
        ...
        const packet = controller.getOutputPacket("lights");
        if (name.match(/grid_/)) {
            HIDDebug("ERROR: set PAD colors with setPADColor");
            return;
        }
        if (!name.match(/.*_brightness$/)) { name = name + "_brightness"; }
        const field = packet.getField("hid", name);
        if (field==undefined) {
    ...
    this.set7SegmentValue = function(name, dp, v1, v2, v3, v4, v5, v6, v7) {
        ...
        field.value = dp;
        field = packet.getField("hid", name+"_segment_1");
        field.value = v1;
        field = packet.getField("hid", name+"_segment_2");
        field.value = v2;
        field = packet.getField("hid", name+"_segment_3");
        field.value = v3;
        field = packet.getField("hid", name+"_segment_4");
        field.value = v4;
        field = packet.getField("hid", name+"_segment_5");
        field.value = v5;
        field = packet.getField("hid", name+"_segment_6");
        field.value = v6;
        field = packet.getField("hid", name+"_segment_7");
        field.value = v7;
    };
    ...
    this.updateLEDs = function() {
        const packet = this.controller.getOutputPacket("lights");
        packet.send();
    };
...
KontrolF1.init = function(id) {
    ...
    const controller = KontrolF1.controller;
    ...
    KontrolF1.segments[3] = [0x40, 0x40, 0x40, 0x40, 0, 0x0, 0x40];
    KontrolF1.segments[4] = [0x40, 0x40, 0x40, 0x0, 0x40, 0x0, 0x0];
    KontrolF1.segments[5] = [0x40, 0x40, 0x0, 0x40, 0x40, 0x0, 0x40];
    KontrolF1.segments[6] = [0x40, 0x40, 0x0, 0x40, 0x40, 0x40, 0x40];
    KontrolF1.segments[7] = [0x0, 0x40, 0x40, 0x40, 0x0, 0x0, 0x0];
...
KontrolF1.ButtonLEDPressUpdate = function(packet, changed_data) {
    let send_led_update = false;
    for (const field in changed_data) {
        const delta = changed_data[field];
        ...
        if (KontrolF1.buttonNames.indexOf(name)!=-1) {
            if (delta.value==1) { KontrolF1.setButtonBrightness(name, 0x7f); }
            if (delta.value==0) { KontrolF1.setButtonBrightness(name, 0); }
            send_led_update = true;
            break;
        }
    ...
    if (send_led_update) { KontrolF1.updateLEDs(); }
};

KontrolF1.disconnectModeLEDs = function(mode) {
    const grid = KontrolF1.grids[mode];
    const buttons = KontrolF1.playbuttons[mode];
    for (var name in grid) {
        var button = grid[name];
        if (button.ledname==undefined) { continue; }
    ...
    for (var name in buttons) {
        ...
        const button_index = parseInt(name.split("_")[1]);
        if (button.ledname==undefined) { continue; }
        engine.connectControl(button.group, button.ledname,
            KontrolF1.setLED, false
        );
...
KontrolF1.connectModeLEDs = function(mode) {
    ...
    for (var name in grid) {
        ...
        if (engine.getValue(button.group, button.ledname)) { value = button.ledcolor; } else { value = 0, 0, 0; }
...
KontrolF1.setLED = function(value, group, key) {
    const grid = KontrolF1.grids[KontrolF1.controlMode];
    const buttons = KontrolF1.playbuttons[KontrolF1.controlMode];
    let matched = false;

    for (var name in grid) {
        var button = grid[name];
        var button_index = parseInt(name.split("_")[1]);
        if (button.group!=group || button.ledname!=key) { continue; }
        if (value) { value = button.ledcolor; } else { value = 0, 0, 0; }
        KontrolF1.setPADColor(button_index, value[0], value[1], value[2]);
        matched = true;
        break;
    }
    for (var name in buttons) {
        var button = buttons[name];
        var button_index = parseInt(name.split("_")[1]);
        if (button.group!=group || button.name!=key) { continue; }
...
KontrolF1.linkKnob = function(mode, knob, group, name, scaler) {
    if (!(mode in KontrolF1.knobs)) { KontrolF1.knobs[mode] = {}; }
    const mapping = {};
    mapping.mode = mode;
    mapping.knob = knob;
    mapping.group = group;
...
KontrolF1.knob = function(field) {
    const controller = KontrolF1.controller;
    const mode = KontrolF1.knobs[KontrolF1.controlMode];
    if (mode==undefined) {
...
KontrolF1.linkFader = function(mode, fader, group, name, scaler, callback) {
    if (!(mode in KontrolF1.faders)) { KontrolF1.faders[mode] = {}; }
    const mapping = {};
    mapping.mode = mode;
    mapping.fader = fader;
    mapping.group = group;
...
KontrolF1.fader = function(field) {
    const controller = KontrolF1.controller;
    const mode = KontrolF1.faders[KontrolF1.controlMode];
    if (mode==undefined) {
        HIDDebug("Fader group not mapped in mode " + KontrolF1.controlMode);
        return;
    }
    const fader = mode[field.name];
    if (fader==undefined) {
        HIDDebug("Fader "+field.name+ " not mapped in " + KontrolF1.controlMode);
        return;
    }
...
KontrolF1.linkGrid = function(mode, button, group, name, toggle, callback, ledcolor, ledname) {
    if (!(mode in KontrolF1.grids)) { KontrolF1.grids[mode] = {}; }
    if (ledname==undefined) {
        if (name.match(/hotcue_[0-9]/)) { ledname = name + "_enabled"; } else { ledname = name; }
    }
...
KontrolF1.grid = function(field) {
    const mode = KontrolF1.grids[KontrolF1.controlMode];
    if (mode==undefined) {
        HIDDebug("Grid button group not mapped in " + KontrolF1.controlMode);
        return;
    }
    const button = mode[field.name];
    if (button==undefined) {
...
KontrolF1.linkPlay = function(mode, button, group, name, toggle, callback, ledname) {
    if (!(mode in KontrolF1.playbuttons)) { KontrolF1.playbuttons[mode] = {}; }
...
KontrolF1.play = function(field) {
    const mode = KontrolF1.playbuttons[KontrolF1.controlMode];
    if (mode==undefined) {
...
KontrolF1.control = function(control, field) {
    ...
    const scaler = KontrolF1.scalers[control.scaler];
...
KontrolF1.button = function(button, field) {
    ...
    if (button.toggle) {
        if (button.name=="play") { controller.togglePlay(button.group, field); } else { controller.toggle(button.group, button.name, field.value); }
    } else {
...
KontrolF1.setControlMode = function(mode) {
    if (mode==KontrolF1.controlMode) { return; }
...
KontrolF1.hotcue = function(button, field) {
    ...
    if (controller.modifiers.get("shift")) { name = button.name + "_clear"; } else { name = button.name + "_activate"; }
