...
function EKSOtusController() {
    ...
    this.registerOutputPackets = function() {
        ...
        offset = 1;
        for (var led_index=1; led_index<=this.wheelLEDCount/2; led_index++) { packet.addOutput("hid", "wheel_" + led_index, offset++, "B"); }
        this.controller.registerOutputPacket(packet);

        packet = new HIDPacket("led_wheel_right", 0x15, undefined, [0x20]);
        offset = 1;
        for (var led_index=this.wheelLEDCount/2+1; led_index<=this.wheelLEDCount; led_index++) { packet.addOutput("hid", "wheel_" + led_index, offset++, "B"); }
        this.controller.registerOutputPacket(packet);
    ...
    this.requestFirmwareVersion = function() {
        const packet = this.controller.getOutputPacket("request_firmware_version");
    ...
    this.setLEDControlMode = function(mode) {
        ...
        const packet = controller.getOutputPacket("set_ledcontrol_mode");
        const field = packet.getField("hid", "mode");
        if (field==undefined) {
    ...
    this.FirmwareVersionResponse = function(packet, delta) {
        const controller = this.controller;
        const field_major = packet.getField("hid", "major");
        const field_minor = packet.getField("hid", "minor");
        ...
        if (controller.activeDeck!=undefined) {
            ...
            const value = controller.LEDColors.amber;
    ...
    this.setTrackpadMode = function(mode) {
        ...
        const packet = this.controller.getOutputPacket("set_trackpad_mode");
        if (packet==undefined) {
    ...
    this.plusMinus1Scaler = function(group, name, value) {
        if (value<32768) { return value/32768-1; } else { return (value-32768)/32768; }
    };
...
EksOtus.init = function(id) {
    ...
    const controller = EksOtus.controller;
    ...
    for (const deck in controller.deckOutputColors) {
...
EksOtus.outputCallback = function(value, group, key) {
    const controller = EksOtus.controller;
    if (group=="deck") {
        if (controller.activeDeck==undefined) { return; }
    ...
    } else { EksOtus.controller.setOutput("deck", key, controller.LEDColors.off, true); }
};
...
EksOtus.wheelScaler = function(group, name, value) {
    ...
    const delta = EksOtus.wheelPosition - value;
    if (delta>32768) { return 0; }
    EksOtus.wheelPosition = value;
    if (delta>-8 && delta<8) { return -delta/4; }
    return -delta/16;
};
...
EksOtus.rate_wheel = function(field) {
    const controller = EksOtus.controller;
    if (controller.activeDeck==undefined) { return; }
    const active_group = controller.resolveGroup(field.group);
    const current = engine.getValue(active_group, "rate");
    if (field.delta<0) { engine.setValue(active_group, "rate", current+0.003); } else { engine.setValue(active_group, "rate", current-0.003); }
};
...
EksOtus.resetWheelLEDs = function(color) {
    const controller = EksOtus.controller;
    if (color==undefined || !(color in controller.LEDColors)) { color = "off"; }
    for (i=1; i<=EksOtus.wheelLEDCount; i++) { controller.setOutput("jog", "wheel_"+i, color, false); }
    EksOtus.updateLEDs(true);
};
...
EksOtus.hotcue = function(field) {
    ...
    let command;
    if (controller.activeDeck==undefined ||
        field.value==controller.buttonStates.released) { return; }
    const active_group = controller.resolveDeckGroup(controller.activeDeck);
    if (controller.modifiers.get("shift")) { command = field.name + "_clear"; } else { command = field.name + "_activate"; }
...
EksOtus.beat_align = function(field) {
    ...
    if (controller.modifiers.get("shift")) {
        ...
        if (field.value==controller.buttonStates.released) { return; }
        if (!engine.getValue(active_group, "quantize")) { engine.setValue(active_group, "quantize", true); } else { engine.setValue(active_group, "quantize", false); }
    }
};
...
EksOtus.pitchSlider = function(field) {
    ...
    if (controller.modifiers.get("pitch")) {
        ...
        if (field.name=="slider_position") {
            ...
            const value = EksOtus.plusMinus1Scaler(
...
EksOtus.deckSwitch = function(field) {
    const controller = EksOtus.controller;
    if (EksOtus.initialized==false) { return; }
...
EksOtus.enableSpinningPlatterLEDs = function() {
    if (EksOtus.activeSpinningPlatterGroup==undefined) { return; }
...
EksOtus.circleLEDs = function(position) {
    ...
    const wheelLEDSplit = 3;
    const wheelLEDGroups = EksOtus.wheelLEDCount/wheelLEDSplit;
    const timeRemaining = ((1-position)*EksOtus.activeTrackDuration) | 0;
    const track_pos = position * EksOtus.activeTrackDuration;
    const revolutions = track_pos / EksOtus.revTime;
    let led_index = (((revolutions-(revolutions|0))*wheelLEDGroups)|0)*wheelLEDSplit;
    led_index++;
    if (led_index==EksOtus.activeSpinningPlatterLED) { return; }
    EksOtus.activeSpinningPlatterLED = led_index;
    EksOtus.resetWheelLEDs("off", false);
    const led_color = controller.deckOutputColors[controller.activeDeck];
