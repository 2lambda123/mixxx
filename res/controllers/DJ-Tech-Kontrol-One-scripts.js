...
KONTROL1.button = function button(channel, control, value, status, group) {
    ...
    const thecontrol=KONTROL1.getControl(control);
    const command = (value==127)?thecontrol[0]:thecontrol[1];
...
KONTROL1.knobRelease = function knobRelease(knobnum) {
    ...
    if (KONTROL1.exitKnobLoop===true && knobnum=="k4") {
        const group=KONTROL1.getGroup("default");
...
KONTROL1.bankIndicators= function bankIndicators() {
    ...
    const cc = KONTROL1.channels[KONTROL1.curChannel].cc;
    const address = KONTROL1.bankIndicatorLED[KONTROL1.channels[KONTROL1.curChannel].curBank];
...
KONTROL1.updateLEDs = function updateLEDs(knobsonly, force) {
    ...
    //updates all LEDs
    let state;
    const samehook=false;
    let hookgroup;
    let midino;
    let control;
    const controllist=(knobsonly===true)?KONTROL1.knobLEDs:KONTROL1.allLEDs;
...
KONTROL1.doLogKnobLEDs = function doLogKnobLEDs(value, group, control) {
    ...
    else if (value<110) { value+=5; } else if (value<120) { value+=2; }
    value=(value>127)?127:value;
    value=(value<0)?0:value;
    if (KONTROL1.debug>2) { print("value: "+value); }
    let midino;
    const ch=KONTROL1.channels[KONTROL1.curChannel].cc;
...
KONTROL1.doHotcueLEDs = function doHotcueLEDs(value, group, control) {
    ...
    const ch=KONTROL1.channels[KONTROL1.curChannel].noteon;
    const midino=KONTROL1.hotcueLEDs[hotcuenum];
...
KONTROL1.loopLenLEDs = function loopLenLEDs(deck, num) {
    ...
    const group=KONTROL1.getGroup(deck);
...
KONTROL1.lightButtonLEDs = function lightButtonLEDs(value, group, control) {
    ...
    if (typeof(KONTROL1.buttonLEDs[control])!== "undefined") {
        const midino=KONTROL1.buttonLEDs[control];
        midi.sendShortMsg(ch, midino, value);
    }
};
...
KONTROL1.LoopBankLEDs = function LoopBankLEDs(value, group, control) {
    ...
    if (typeof(KONTROL1.loopbuttonLEDs[control])!== "undefined") {
        const midino=KONTROL1.loopbuttonLEDs[control];
        midi.sendShortMsg(ch, midino, value);
    }
};
...
KONTROL1.scratchParameterLEDs = function scratchParameterLEDs(midino, thearray, theindex) { //lights knobs for scratch parameter adjustment
    ...
    let state=Math.round((127/thearray.length)*theindex);
...
KONTROL1.logKnobAdjust = function logKnobAdjust(group, control, minRange, maxRange, value) {
    ...
    //K = 4*(V)^2
    //V = (K/4)^0.5 -- K= value coming from prog (0-4, logarithmic scale), V=value adjusted to linear 0-1
    const curValue=engine.getValue(KONTROL1.getGroup(group), control);
    const adjustedValue=Math.pow((curValue/4), .5);//adjust range so it's on a scale of 0 to 1 - logarithmic

    const inc=(value==127)?.02:-.02;//increment - how far to move on each knob click

    let newValue=4*Math.pow((adjustedValue+inc), 2);
...
KONTROL1.cueLoop = function cueLoop(cue, len) { //jump to hotcue and start loop
    ...
    if (engine.getValue(group, "hotcue_"+cue+"_enabled")!==1) { //set hotcue and start loop
        ...
        const startpos=engine.getValue(group, "hotcue_"+cue+"_position");
        const loopseconds = len*(1/(engine.getValue(group, "bpm")/60));
        const loopsamples = loopseconds*engine.getValue(group, "track_samplerate")*2;//*2 to compensate for stereo samples
...
KONTROL1.play = function play(value, deck) {
    ...
    deck=(typeof deck !== "undefined")?deck:"default";
    const group=KONTROL1.getGroup(deck);
    const state=(engine.getValue(group, "play")==1)?0:1;
...
KONTROL1.beatjump = function(jumplen) { //jumps back jumplen beats
    ...
    const curpos = engine.getValue(group, "playposition")*engine.getValue(group, "track_samples");
    const numbeats = jumplen;
    const backseconds = numbeats*(1/(engine.getValue(group, "bpm")/60));
    const backsamples = backseconds*engine.getValue(group, "track_samplerate")*2;//*2 to compensate for stereo samples
    const newpos = curpos-(backsamples);
...
KONTROL1.looplenDial = function looplenDial(value, deck) {
    ...
    let arraypointer=false;
    let inc=(value==127)?1:-1;//increment - how far to move on each knob click
    for (i=1; i<12; i++) {
    ...
    const newLoopLen=KONTROL1.beatloopLengths[arraypointer];
...
KONTROL1.wheelMoveLoop = function wheelMoveLoop(val) {
    ...
    var val=(val>64)?-1:1;
    const interval =    val*KONTROL1.wheelloopmove*engine.getValue(group, "track_samples")/engine.getValue(group, "duration");
...
KONTROL1.resumeUnset = function resumeUnset(value, group, control) {
    ...
    let ch;
    for (const k in KONTROL1.defaultDeck) {
...
KONTROL1.resume = function resume(deck) {
    ...
    if (KONTROL1.channels[KONTROL1.curChannel].resumepos>-1) {
        const oldpos = KONTROL1.channels[KONTROL1.curChannel].resumepos*engine.getValue(group, "track_samples");
        const seconds = (Date.now()- KONTROL1.channels[KONTROL1.curChannel].resumetime)/1000;//get elapsed time in seconds
        const samples = seconds*engine.getValue(group, "track_samplerate")*2;//how many samples to jump forward - multiplied by 2 for stereo samples
        const newpos = oldpos + samples;//new position in samples
        engine.setValue(group, "playposition", newpos/engine.getValue(group, "track_samples"));
...
KONTROL1.setScratchRev = function setScratchRev(value) { //set scratch parameter - record RPM
    ...
    const inc=(value==127)?1:-1;//increment - how far to move on each knob click
...
KONTROL1.getControl=function getControl(control) {
    ...
    const bank=KONTROL1.getBankCode();//check for setting on current bank with mods first
    const nomodbank=KONTROL1.getNoModBankCode();//then current bank without mods
    const noshiftbank=KONTROL1.getNoShiftBankCode();//then for setting without shift button.  If nothing found, then use default bank.
    const noshiftnomodbank=KONTROL1.getNoShiftNoModBankCode();//then for setting without shift button.  If nothing found, then use default bank.
    const defaultbankwmod="bank1a-"+KONTROL1.getModCode();
    const defaultbank="bank1a-ooooooo";
...
KONTROL1.getModCode = function getModCode() { //returns the mod code to use as part of the key to access the controls object
    ...
    const outstr=KONTROL1.mod.p1.state+KONTROL1.mod.p2.state+KONTROL1.mod.k1.state+KONTROL1.mod.k2.state+KONTROL1.mod.k3.state+KONTROL1.mod.k4.state+KONTROL1.mod.s.state;
    return outstr;
};
...
KONTROL1.getBankCode = function getBankCode() { //returns the bank code (with mods) to use as a key to access the controls object
    ...
    const outstr=KONTROL1.channels[KONTROL1.curChannel].curBank+KONTROL1.channels[KONTROL1.curChannel].curShift+"-"+KONTROL1.getModCode();
    return outstr;
};
...
KONTROL1.getNoModBankCode = function getNoModBankCode() { //return current bank with no mods
    ...
    const outstr=KONTROL1.channels[KONTROL1.curChannel].curBank+KONTROL1.channels[KONTROL1.curChannel].curShift+"-ooooooo";
    return outstr;
};
...
KONTROL1.getNoShiftBankCode = function getNoShiftBankCode() { //return current bank with mods but no shift
    ...
    const outstr=KONTROL1.channels[KONTROL1.curChannel].curBank+"a-"+KONTROL1.getModCode();
    return outstr;
};
...
KONTROL1.getNoShiftNoModBankCode = function getNoShiftNoModBankCode() { //return current bank with no shifts or mods
    ...
    const outstr=KONTROL1.channels[KONTROL1.curChannel].curBank+"a-ooooooo";
    return outstr;
};
...
KONTROL1.getGroup = function getGroup(groupstr) { //return Mixxx group
    ...
    let outstr;
    if (groupstr=="default") { outstr=KONTROL1.defaultDeck[KONTROL1.curChannel]; } else if (groupstr=="alt") { outstr=KONTROL1.altDeck[KONTROL1.curChannel]; } else if (groupstr=="sync") { outstr=KONTROL1.syncDeck[KONTROL1.curChannel]; } else { outstr=groupstr; }
...
KONTROL1.getFunctionName = function getFunctionName() {
    const re = /function (.*?)\(/;
    const s = KONTROL1.getFunctionName.caller.toString();
    const m = re.exec(s);
    return m[1];
};
