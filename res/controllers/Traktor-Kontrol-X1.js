...
KontrolX1.shutdown = function() {
    for (const led_name in KontrolX1.ledmap) {
        const led = KontrolX1.ledmap[led_name];
...
KontrolX1.registerControls = function() {
    ...
    KontrolX1.addEncoder(0x10, "[Playlist]", "SelectPlaylistKnob", KontrolX1.SelectPlaylistKnob);
    KontrolX1.addButton(0x12, "[Playlist]", "ToggleSelectedSidebarItem");
    ...
    KontrolX1.addEncoder(0x1a, "[Channel1]", "jog", KontrolX1.jog_encoder);
    KontrolX1.addButton(0x18, "[Channel1]", "rate_reset", true, KontrolX1.rate_reset);
    KontrolX1.addEncoder(0x1b, "[Channel2]", "jog", KontrolX1.jog_encoder);
    ...
    KontrolX1.addLEDButton(0x40, "[Channel1]", "beats_translate_curpos", true);
    KontrolX1.addLEDButton(0x41, "[Channel2]", "beats_translate_curpos", true);
    KontrolX1.addLEDButton(0x42, "[Channel1]", "slip_enabled", true);
    KontrolX1.addLEDButton(0x43, "[Channel2]", "slip_enabled", true);
    ...
    KontrolX1.addLEDButton(0x20, "[Channel1]", "seek_back", true, KontrolX1.seek);
    KontrolX1.addLEDButton(0x21, "[Channel2]", "seek_back", true, KontrolX1.seek);
    KontrolX1.addLEDButton(0x22, "[Channel1]", "seek_fwd", true, KontrolX1.seek);
    KontrolX1.addLEDButton(0x23, "[Channel2]", "seek_fwd", true, KontrolX1.seek);
    KontrolX1.addLEDButton(0x24, "[Channel1]", "cue_default", true);
    KontrolX1.addLEDButton(0x25, "[Channel2]", "cue_default", true);
    ...
    KontrolX1.addLEDButton(0x2a, "[Channel1]", "beatsync", true);
    KontrolX1.addLEDButton(0x2b, "[Channel2]", "beatsync", true);

    KontrolX1.addLEDButton(0x48, "[Channel1]", "loop_halve", true);
    KontrolX1.addLEDButton(0x49, "[Channel2]", "loop_halve", true);
    KontrolX1.addLEDButton(0x4a, "[Channel1]", "loop_double", true);
    KontrolX1.addLEDButton(0x4b, "[Channel2]", "loop_double", true);
    KontrolX1.addLEDButton(0x4c, "[Channel1]", "back", true);
    KontrolX1.addLEDButton(0x4d, "[Channel2]", "back", true);
    KontrolX1.addLEDButton(0x4e, "[Channel1]", "fwd", true);
    KontrolX1.addLEDButton(0x4f, "[Channel2]", "fwd", true);

    KontrolX1.addLEDButton(0x50, "[Channel1]", "beatlooproll_0.25_activate", true);
    KontrolX1.addLEDButton(0x51, "[Channel2]", "beatlooproll_0.25_activate", true);
    KontrolX1.addLEDButton(0x52, "[Channel1]", "beatlooproll_0.5_activate", true);
    KontrolX1.addLEDButton(0x53, "[Channel2]", "beatlooproll_0.5_activate", true);
    KontrolX1.addLEDButton(0x54, "[Channel1]", "beatlooproll_1_activate", true);
...
KontrolX1.setLED = function(value, group, key) {
    const led_id = group+"."+key;
    const led = KontrolX1.ledmap[led_id];
    if (led==undefined) {
...
KontrolX1.buttons = function(channel, control, value, status, group) {
    const button = KontrolX1.buttonmap[control];
    const shift = (control>=0x2c);
    ...
    if (!button.hold && value==0) { return; }

    if (button.control=="play") {
        if (engine.getValue(button.group, "play")) { engine.setValue(button.group, "play", false); } else { engine.setValue(button.group, "play", true); }
        return;
    }
...
KontrolX1.addKnob = function(index, group, control, scaler, callback) {
    if (scaler==undefined && control in KontrolX1.scalers) { scaler = control; }
...
KontrolX1.knobs = function(channel, control, value, status, group) {
    const knob = KontrolX1.knobmap[control];
    const shift = (control>=0x2c);
    if (knob==undefined) {
...
KontrolX1.encoders = function(channel, control, value, status, group) {
    const encoder = KontrolX1.encodermap[control];
    const shift = (control>=0x2c);
...
KontrolX1.loop_size = function(channel, control, value, status, group) {
    switch (value) {
    case -1:
        control = "loop_halve"; break;
    case 1:
        control = "loop_double"; break;
    default:
        KontrolX1.debug("Unknown value for rate_encoder: " + value);
        return;
    }
