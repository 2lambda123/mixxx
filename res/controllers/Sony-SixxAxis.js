...
SonySixxAxis.init = function(id) {
    SonySixxAxis.id = id;
    const controller = SonySixxAxis.controller;
...
SonySixxAxis.registerCallbacks = function(id) {
    const controller = SonySixxAxis.controller;
    const packet = SonySixxAxis.controller.getInputPacket("control");
    if (packet==undefined) {
...
SonySixxAxis.front_left = function(field) {
    ...
    if (!controller.modifiers.get("shift_left")) { return; }
    if (field.name=="button_top_left") { engine.setValue("[Playlist]", "SelectPrevPlaylist", true); }
    if (field.name=="button_bottom_left") { engine.setValue("[Playlist]", "SelectNextPlaylist", true); }
};

SonySixxAxis.front_right = function(field) {
    const controller = SonySixxAxis.controller;
    if (field.value==controller.buttonStates.released) { return; }
    if (!controller.modifiers.get("shift_right")) { return; }
...
SonySixxAxis.left_jog = function(field) {
    const controller = SonySixxAxis.controller;
    const group = controller.resolveGroup("deck1");
    if (group==undefined) { return; }
    if (field.name=="jog_left_y") {
        if (field.delta<=0) { return; }
        old_value = engine.getValue(group, "rate");
        if (field.value<115) { engine.setValue(group, "rate", old_value+0.005); } else if (field.value>135) { engine.setValue(group, "rate", old_value-0.005); }
    }
...
SonySixxAxis.right_jog = function(field) {
    const controller = SonySixxAxis.controller;
    const group = controller.resolveGroup("deck2");
...
SonySixxAxis.jog_bend = function(group, direction, value) {
    const controller = SonySixxAxis.controller;
    const jog_value = value/32;
    group = controller.resolveGroup(group);
    if (group==undefined) { return; }
    if (direction=="up") { engine.setValue(group, "jog", 6); }
    if (direction=="down") { engine.setValue(group, "jog", -6); }
};
