...
mc3000.rate = function(channel, control, value, status, group) {
    ...
    const MSB = value;
    const LSB = control;
    const num14 = (MSB << 7) | LSB;  // Construct the 14-bit number
    const nrate = (num14-8192)/8192;
...
mc3000.cue_default = function(channel, control, value, status, group) {
    const isPressed = (status & 0xF0)==0x90 ? 1 : 0;
...
mc3000.hotcueset = function(channel, control, value, status, group) {
    ...
    const nocue=mc3000.hotcues[control];
    const is_play = engine.getValue(group, "play")==1;
...
mc3000.setLoopAtHotCue = function(group, nohotcue, nbbeat) {
    const position = engine.getValue(group, "hotcue_"+nohotcue+"_position");
    const deck = mc3000.groupToDeck(group);
...
mc3000.shift = function(channel, control, value, status, group) {
    mc3000.state["shift"+mc3000.groupToDeck(group)] = (status & 0xF0)==0x90; // 1 if pressed
};
...
mc3000.fxon1 = function(channel, control, value, status, group) {
    const newValue = !engine.getValue(group, "flanger");
...
mc3000.fxon2 = function(channel, control, value, status, group) {
    if (mc3000.state["shift"+mc3000.groupToDeck(group)]) {
        ...
        const newValue = !engine.getValue(group, "quantize");
...
mc3000.wheelTouch = function(channel, control, value, status, group) {
    ...
    if (mc3000.scratch[deck-1]) {
        ...
        mc3000.scratchpressed[deck-1] = (status & 0xF0)==0x90;
    }
};
...
mc3000.jogWheel = function(channel, control, value, status, group) {
    const deck = mc3000.groupToDeck(group);
    let adjustedJog = value - 64; // Control centers on 0x40 (64)
    ...
    const gammaInputRange = 64;    // Max jog speed
    const maxOutFraction = 0.5;    // Where on the curve it should peak; 0.5 is half-way
    const sensitivity = 0.5;        // Adjustment gamma 0.5 def
...
mc3000.efx_beatLoopX = function(channel, control, value, status, group) {
    const deck = mc3000.groupToDeck(group);
    const nbbeat = mc3000.efx2no[(control-18)-((deck-1)*64)];
...
mc3000.autoLoop = function(channel, control, value, status, group) {
    ...
    mc3000.state["alshift"+deck] = (status & 0xF0)==0x90; // true if pressed
};
...
mc3000.loopStartSetLed = function(value, group) {
    mc3000.setled(mc3000.groupToDeck(group), mc3000.leds.loopin, value != -1);
};

mc3000.loopEndSetLed = function(value, group) {
    mc3000.setled(mc3000.groupToDeck(group), mc3000.leds.loopout, value != -1);
};
...
mc3000.vuMeterSetLeds = function(value, group) {
    const deck = mc3000.groupToDeck(group);
    const maxled = Math.round((value-0.07)*7);
    const curled = mc3000.vumetermaxled[deck-1];
    if (maxled !== curled) {
        let i=0;
        const offset = mc3000.vumeteroffset[deck-1];
        if (maxled > curled) {
