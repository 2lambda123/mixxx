...
StantonSCS1m.scratch = {
    "rpm": (33+1/3)*StantonSCS1m.scratchFactor,
    "resolution": 30, "alpha": 1.0/8, "beta": (1.0/8)/32
};
StantonSCS1m.hotCueDeck = 1;    // Current hot cue page
StantonSCS1m.hotCues =  {
    1: {35: 1, 36: 2, 37: 3, 38: 4},
...
StantonSCS1m.init = function(id) {    // called when the MIDI device is opened & set up
    ...
    const No = 0x90 + StantonSCS1m.channel;
    ...
    const LEDInit = [["Channels", "rate_temp_up"],
    ...
    for (i=0; i<LEDInit.length; i++) {
        const group = LEDInit[i][0];
        const name = LEDInit[i][1];
        if (group=="Channels") {
...
StantonSCS1m.connectPresetSignals = function(channel, disconnect) {
    ...
    const signalList = StantonSCS1m.hotcueSignals;
    for (i=0; i<signalList.length; i++) {
        ...
        if (StantonSCS1m.debug) {
            if (disconnect) { print("StantonSCS1m: "+group+","+signalList[i][1]+" disconnected from "+signalList[i][2]); } else { print("StantonSCS1m: "+group+","+signalList[i][1]+" connected to "+signalList[i][2]); }
        }
    }
    // If disconnecting signals, darken the LEDs on the preset buttons
    if (disconnect) {
        const No = 0x80 + channel;
...
StantonSCS1m.playButton = function(channel, control, value, status, deck) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        const currentlyPlaying = engine.getValue("[Channel"+deck+"]", "play");
...
StantonSCS1m.setupButton = function(channel, control, value, status) {
    if ((status & 0XF0) == 0x90) { StantonSCS1m.inSetup = !StantonSCS1m.inSetup; } else if (StantonSCS1m.inSetup) {  // If entering setup, change the LCD back light colors to green
...
StantonSCS1m.selectKnob = function(channel, control, value, status) {
    ...
    case "control":
        ...
        if (StantonSCS1m.modifier.enterButton==1 || StantonSCS1m.modifier.cancelButton==1) { engine.scratchTick(StantonSCS1m.scratchDeck, jogValue); }
        break;
    case "browse":
...
StantonSCS1m.pitchRangeKnob = function(value, deck) {
    if (StantonSCS1m.checkInSetup()) { return; }
    const currentValue = engine.getValue("[Channel"+deck+"]", "rateRange");
    let newValue = Math.round(currentValue*100+(value-64))*0.01;
...
StantonSCS1m.faderStartToggle = function(channel, control, value, status) {
    ...
    const No = 0x90 + channel;
    if (StantonSCS1m.faderStart) { midi.sendShortMsg(No, 8, 1); } else { midi.sendShortMsg(No, 8, 0); }
};
...
StantonSCS1m.hotCueDeckChange = function(channel, control, value, status) {
    ...
    if ((status & 0XF0) == 0x90) {    // If button down
        ...
        if (StantonSCS1m.hotCueDeck == engine.getValue("[Master]", "num_decks")) { StantonSCS1m.hotCueDeck=1; } else { StantonSCS1m.hotCueDeck++; }
...
StantonSCS1m.presetButton = function(channel, control, value, status) {
    if (StantonSCS1m.checkInSetup()) { return; }
    const deck = StantonSCS1m.hotCueDeck;
    if ((status & 0xF0) == 0x90) {    // If button down
        // Multiple cue points
...
StantonSCS1m.buttonLED = function(value, note, on, off) {
    const byte1 = 0x90 + StantonSCS1m.channel;
...
StantonSCS1m.PresetLED = function(value, control) {
    if (value>0) { midi.sendShortMsg(0x90 + StantonSCS1m.channel, control, 64); } else { midi.sendShortMsg(0x80 + StantonSCS1m.channel, control, 0); }
};
...
StantonSCS1m.clipLED = function(value, note) {
    const on = 0x90 + StantonSCS1m.channel;
    const off = 0x80 + StantonSCS1m.channel;
    if (value>0) { midi.sendShortMsg(on, note, 8); } else { midi.sendShortMsg(off, note, 8); }
};

StantonSCS1m.VuMeter = function(value, note) {
    const range = 1/7;    // keep the highest one for the clip signal
    let newLEDs=0;
    if (value>=range*7) { newLEDs = 255; } else if (value>range*6) { newLEDs = 127; } else if (value>range*5) { newLEDs = 63; } else if (value>range*4) { newLEDs = 31; } else if (value>range*3) { newLEDs = 15; } else if (value>range*2) { newLEDs = 7; } else if (value>range) { newLEDs = 3; } else if (value>0.001) { newLEDs = 1; }

    // Last one XOR this one gives the LEDs that changed state
    const change = StantonSCS1m.lastVu[note] ^ newLEDs;
...
StantonSCS1m.pitchDisplay = function(value, deck) {
    let data = (value * engine.getValue("[Channel"+deck+"]", "rateRange") * 100);
...
StantonSCS1m.pitchColor = function(value, deck) {
    ...
    if (value<=0.25) { midi.sendShortMsg(No, 49+offset, 96); } else if (value<=0.5) { midi.sendShortMsg(No, 49+offset, 127); } else if (value>0.5) { midi.sendShortMsg(No, 49+offset, 32); }
...
StantonSCS1m.positionUpdates = function(value, deck) {
    const No = 0x90 + StantonSCS1m.channel;
    const trackTimeRemaining = ((1-value) * StantonSCS1m.trackDuration[deck]) | 0;    // OR with 0 replaces Math.floor and is faster
    ...
    const revtime = StantonSCS1m.revtime;
    const currentTrackPos = value * StantonSCS1m.trackDuration[deck];

    const revolutions = currentTrackPos/revtime;
    const light = ((revolutions-((revolutions)|0))*18) | 0; // OR with 0 replaces Math.floor and is faster
    ...
    if (StantonSCS1m.lastLight[deck]!=light) {  // Don't send light commands if there's no visible change
        const byte1 = 0xB0 + StantonSCS1m.channel;
        if (deck==1) { midi.sendShortMsg(byte1, 0x7E, light+1); } else { midi.sendShortMsg(byte1, 0x7D, light+1); }
    ...
    if (!StantonSCS1m.inSetup) {    // If not in setup mode
        ...
        const message = "-"+secondstominutes(trackTimeRemaining);
