...
VestaxVCI400.Deck.prototype.onVuMeterChanged = function(value, group, key) {
    ...
    const statusByte = "0xb".concat(midiNo.toString());
...
VestaxVCI400.Deck.prototype.onHotCue1Changed = function(value, group, key) {
    try {
        const deck = VestaxVCI400.GetDeck(group);
...
VestaxVCI400.fx1ToggleButton3 = function(channel, control, value, status, group) {
    ...
    const curval = engine.getValue(group, "quantize");
...
VestaxVCI400.fx1ToggleButton4 = function(channel, control, value, status, group) {
    if (value == 0) { return; }
    var group = VestaxVCI400.Decks.A.isActive ? "[Channel1]" : "[Channel3]";
    const curval = engine.getValue(group, "keylock");
...
VestaxVCI400.fx1Knob = function(channel, control, value, status, group) {
    let mixVal = engine.getValue("[EffectRack1_EffectUnit1]", "mix");
    if (value == 0x01) {
        mixVal += 0.05;
    } else {
        mixVal -= 0.05;
...
VestaxVCI400.fx2Knob = function(channel, control, value, status, group) {
    let mixVal = engine.getValue("[EffectRack1_EffectUnit2]", "mix");
    if (value == 0x01) {
        mixVal += 0.05;
    } else {
        mixVal -= 0.05;
...
VestaxVCI400.loopKnob = function(channel, control, value, status, group) {
    try {
        const deck = VestaxVCI400.GetDeck(group);
        const isLoopActive = engine.getValue(deck.group, "loop_enabled");
        const jogValue = value - 0x40; // -64 to +63, - = CCW, + = CW

        if (isLoopActive) {
            if (VestaxVCI400.shiftActive) {
                if (jogValue > 0) { engine.setValue(deck.group, "loop_move", -1); } else { engine.setValue(deck.group, "loop_move", 1); }
            } else {
                if (jogValue > 0) {
...
VestaxVCI400.pitchKnob = function(channel, control, value, status, group) {
    try {
        ...
        if (VestaxVCI400.shiftActive) {
            let playPosition = engine.getValue(deck.group, "playposition");
            if (jogValue > 0) {
                playPosition -= 0.0125;
            } else {
                playPosition += 0.0125;
